-- autogenerated lush spec on Tue 23 Jul 2024 03:34:24 PM IST
-- Run :Lushify
-- then /Normal to find the Normal group and edit your fg & bg colors
local lush = require("lush")
--local hsl = lush.hsl
--local hsluv = lush.hsluv
local theme = lush(function(injected_functions)
  local sym = injected_functions.sym
  return {
    SpecialKey { fg = "#8b7079", },                                                        -- SpecialKey     xxx guifg=#8b7079
    EndOfBuffer { fg = "#393337", },                                                       -- EndOfBuffer    xxx guifg=#393337
    NeoTreeEndOfBuffer { EndOfBuffer },                                                    -- NeoTreeEndOfBuffer xxx links to EndOfBuffer
    TermCursor { gui = "reverse", },                                                       -- TermCursor     xxx cterm=reverse gui=reverse
    NonText { fg = "#393337", },                                                           -- NonText        xxx guifg=#393337
    Whitespace { NonText },                                                                -- Whitespace     xxx links to NonText
    LspInlayHint { NonText },                                                              -- LspInlayHint   xxx links to NonText
    TelescopePromptCounter { NonText },                                                    -- TelescopePromptCounter xxx links to NonText
    TelescopeResultsDiffUntracked { NonText },                                             -- TelescopeResultsDiffUntracked xxx links to NonText
    TelescopePreviewHyphen { NonText },                                                    -- TelescopePreviewHyphen xxx links to NonText
    Directory { fg = "#9cbdc9", },                                                         -- Directory      xxx guifg=#9cbdc9
    NeoTreeDirectoryName { Directory },                                                    -- NeoTreeDirectoryName xxx links to Directory
    TelescopePreviewDate { Directory },                                                    -- TelescopePreviewDate xxx links to Directory
    TelescopePreviewDirectory { Directory },                                               -- TelescopePreviewDirectory xxx links to Directory
    ErrorMsg { fg = "#df8489", },                                                          -- ErrorMsg       xxx guifg=#df8489
    NvimInvalidSpacing { ErrorMsg },                                                       -- NvimInvalidSpacing xxx links to ErrorMsg
    MasonError { ErrorMsg },                                                               -- MasonError     xxx links to ErrorMsg
    IncSearch { bg = "#efbde3", fg = "#1e1517", },                                         -- IncSearch      xxx guifg=#1e1517 guibg=#efbde3
    CurSearch { IncSearch },                                                               -- CurSearch      xxx links to IncSearch
    HopPreview { IncSearch },                                                              -- HopPreview     xxx links to IncSearch
    Search { bg = "#1b282c", },                                                            -- Search         xxx guibg=#1b282c
    NeoTreePreview { Search },                                                             -- NeoTreePreview xxx links to Search
    TelescopePreviewMatch { Search },                                                      -- TelescopePreviewMatch xxx links to Search
    MoreMsg { gui = "bold", fg = "#8b7079", },                                             -- MoreMsg        xxx cterm=bold gui=bold guifg=#8b7079
    ModeMsg { gui = "bold", fg = "#8b7079", },                                             -- ModeMsg        xxx cterm=bold gui=bold guifg=#8b7079
    LineNr { fg = "#393337", },                                                            -- LineNr         xxx guifg=#393337
    LineNrAbove { LineNr },                                                                -- LineNrAbove    xxx links to LineNr
    LineNrBelow { LineNr },                                                                -- LineNrBelow    xxx links to LineNr
    LazyProgressTodo { LineNr },                                                           -- LazyProgressTodo xxx links to LineNr
    TelescopeResultsLineNr { LineNr },                                                     -- TelescopeResultsLineNr xxx links to LineNr
    CursorLineNr { fg = "#dadada", },                                                      -- CursorLineNr   xxx guifg=#dadada
    Question { fg = "#efbde3", },                                                          -- Question       xxx guifg=#efbde3
    StatusLine { bg = "#281e22", fg = "#8b7079", },                                        -- StatusLine     xxx guifg=#8b7079 guibg=#281e22
    MsgSeparator { StatusLine },                                                           -- MsgSeparator   xxx links to StatusLine
    NeoTreeStatusLine { StatusLine },                                                      -- NeoTreeStatusLine xxx links to StatusLine
    StatusLineNC { fg = "#68545b", },                                                      -- StatusLineNC   xxx guifg=#68545b
    NeoTreeStatusLineNC { StatusLineNC },                                                  -- NeoTreeStatusLineNC xxx links to StatusLineNC
    WinSeparator { bg = "#1e1517", fg = "#281e22", },                                      -- WinSeparator   xxx guifg=#281e22 guibg=#1e1517
    VertSplit { bg = "#1e1517", fg = "#281e22", },                                         -- VertSplit      xxx guifg=#281e22 guibg=#1e1517
    Title { gui = "bold", bg = "#1a1113", fg = "#9cbdc9", },                               -- Title          xxx cterm=bold gui=bold guifg=#9cbdc9 guibg=#1a1113
    DapUIBreakpointsPath { Title },                                                        -- DapUIBreakpointsPath xxx links to Title
    DapUIStoppedThread { Title },                                                          -- DapUIStoppedThread xxx links to Title
    DapUIScope { Title },                                                                  -- DapUIScope     xxx links to Title
    LspInfoTitle { Title },                                                                -- LspInfoTitle   xxx links to Title
    NullLsInfoSources { Title },                                                           -- NullLsInfoSources xxx links to Title
    Visual { bg = "#1b282c", },                                                            -- Visual         xxx guibg=#1b282c
    LspSignatureActiveParameter { Visual },                                                -- LspSignatureActiveParameter xxx links to Visual
    SnippetTabstop { Visual },                                                             -- SnippetTabstop xxx links to Visual
    LazyButtonActive { Visual },                                                           -- LazyButtonActive xxx links to Visual
    UfoFoldedBg { Visual },                                                                -- UfoFoldedBg    xxx links to Visual
    UfoPreviewCursorLine { Visual },                                                       -- UfoPreviewCursorLine xxx links to Visual
    TelescopePreviewLine { Visual },                                                       -- TelescopePreviewLine xxx links to Visual
    WarningMsg { fg = "#ffd770", },                                                        -- WarningMsg     xxx guifg=#ffd770
    MasonWarning { WarningMsg },                                                           -- MasonWarning   xxx links to WarningMsg
    WildMenu { bg = "#9cbdc9", fg = "#1e1517", },                                          -- WildMenu       xxx guifg=#1e1517 guibg=#9cbdc9
    Folded { fg = "#393337", },                                                            -- Folded         xxx guifg=#393337
    FoldColumn { fg = "#393337", },                                                        -- FoldColumn     xxx guifg=#393337
    CursorLineFold { FoldColumn },                                                         -- CursorLineFold xxx links to FoldColumn
    DiffAdd { bg = "#6d9f71", fg = "#1e1517", },                                           -- DiffAdd        xxx guifg=#1e1517 guibg=#6d9f71
    TelescopeResultsDiffAdd { DiffAdd },                                                   -- TelescopeResultsDiffAdd xxx links to DiffAdd
    DiffChange { bg = "#ffd770", fg = "#1e1517", },                                        -- DiffChange     xxx guifg=#1e1517 guibg=#ffd770
    sym "@diff.delta" { DiffChange },                                                      -- @diff.delta    xxx links to DiffChange
    TelescopeResultsDiffChange { DiffChange },                                             -- TelescopeResultsDiffChange xxx links to DiffChange
    DiffDelete { bg = "#df8489", fg = "#1e1517", },                                        -- DiffDelete     xxx guifg=#1e1517 guibg=#df8489
    sym "@diff.minus" { DiffDelete },                                                      -- @diff.minus    xxx links to DiffDelete
    TelescopeResultsDiffDelete { DiffDelete },                                             -- TelescopeResultsDiffDelete xxx links to DiffDelete
    DiffText { bg = "#ffd770", fg = "#1e1517", },                                          -- DiffText       xxx guifg=#1e1517 guibg=#ffd770
    Conceal { fg = "#68545b", },                                                           -- Conceal        xxx guifg=#68545b
    LazyDimmed { Conceal },                                                                -- LazyDimmed     xxx links to Conceal
    LazyProp { Conceal },                                                                  -- LazyProp       xxx links to Conceal
    SpellBad { gui = "undercurl", sp = "#e77777", },                                       -- SpellBad       xxx cterm=undercurl gui=undercurl guisp=#e77777
    SpellCap { gui = "undercurl", sp = "#ffd770", },                                       -- SpellCap       xxx cterm=undercurl gui=undercurl guisp=#ffd770
    SpellRare { gui = "undercurl", sp = "#9ac374", },                                      -- SpellRare      xxx cterm=undercurl gui=undercurl guisp=#9ac374
    SpellLocal { gui = "undercurl", sp = "#9cbdc9", },                                     -- SpellLocal     xxx cterm=undercurl gui=undercurl guisp=#9cbdc9
    Pmenu { bg = "#1a1113", fg = "#8b7079", },                                             -- Pmenu          xxx guifg=#8b7079 guibg=#1a1113
    PmenuKind { Pmenu },                                                                   -- PmenuKind      xxx links to Pmenu
    PmenuExtra { Pmenu },                                                                  -- PmenuExtra     xxx links to Pmenu
    PmenuSel { gui = "bold", bg = "#1b282c", blend = 0, },                                 -- PmenuSel       xxx cterm=bold gui=bold guibg=#1b282c blend=0
    PmenuKindSel { PmenuSel },                                                             -- PmenuKindSel   xxx links to PmenuSel
    PmenuExtraSel { PmenuSel },                                                            -- PmenuExtraSel  xxx links to PmenuSel
    TelescopeSelection { PmenuSel },                                                       -- TelescopeSelection xxx links to PmenuSel
    NeoTreeCursorLine { PmenuSel },                                                        -- NeoTreeCursorLine xxx links to PmenuSel
    PmenuSbar { bg = "#1a1113", },                                                         -- PmenuSbar      xxx guibg=#1a1113
    UfoPreviewSbar { PmenuSbar },                                                          -- UfoPreviewSbar xxx links to PmenuSbar
    PmenuThumb { bg = "#9cbdc9", blend = 0, },                                             -- PmenuThumb     xxx guibg=#9cbdc9 blend=0
    UfoPreviewThumb { PmenuThumb },                                                        -- UfoPreviewThumb xxx links to PmenuThumb
    TabLine { bg = "#281e22", fg = "#68545b", },                                           -- TabLine        xxx guifg=#68545b guibg=#281e22
    TabLineSel { gui = "bold,italic", bg = "#1e1517", fg = "#dadada", },                   -- TabLineSel     xxx cterm=bold,italic gui=bold,italic guifg=#dadada guibg=#1e1517
    TabLineFill { bg = "#281e22", },                                                       -- TabLineFill    xxx guibg=#281e22
    CursorColumn { bg = "#22191b", },                                                      -- CursorColumn   xxx guibg=#22191b
    CursorLine { bg = "#22191b", },                                                        -- CursorLine     xxx guibg=#22191b
    MasonMutedBlock { CursorLine },                                                        -- MasonMutedBlock xxx links to CursorLine
    LazyButton { CursorLine },                                                             -- LazyButton     xxx links to CursorLine
    UfoCursorFoldedLine { CursorLine },                                                    -- UfoCursorFoldedLine xxx links to CursorLine
    ColorColumn { bg = "#22191b", },                                                       -- ColorColumn    xxx guibg=#22191b
    QuickFixLine { bg = "#ffd770", fg = "#1e1517", },                                      -- QuickFixLine   xxx guifg=#1e1517 guibg=#ffd770
    NormalNC { bg = "#1a1113", fg = "#dadada", },                                          -- NormalNC       xxx guifg=#dadada guibg=#1a1113
    NormalFloat { bg = "#1a1113", fg = "#8b7079", },                                       -- NormalFloat    xxx guifg=#8b7079 guibg=#1a1113
    LazyNormal { NormalFloat },                                                            -- LazyNormal     xxx links to NormalFloat
    WhichKeyFloat { NormalFloat },                                                         -- WhichKeyFloat  xxx links to NormalFloat
    MasonNormal { NormalFloat },                                                           -- MasonNormal    xxx links to NormalFloat
    NullLsInfoBorder { NormalFloat },                                                      -- NullLsInfoBorder xxx links to NormalFloat
    NeoTreeFloatNormal { NormalFloat },                                                    -- NeoTreeFloatNormal xxx links to NormalFloat
    FloatBorder { bg = "#1a1113", fg = "#736468", },                                       -- FloatBorder    xxx guifg=#736468 guibg=#1a1113
    WhichKeyBorder { FloatBorder },                                                        -- WhichKeyBorder xxx links to FloatBorder
    DapUIFloatBorder { FloatBorder },                                                      -- DapUIFloatBorder xxx links to FloatBorder
    WinBar { fg = "#8d8184", },                                                            -- WinBar         xxx guifg=#8d8184
    WinBarNC { bg = "#1a1113", fg = "#68545b", },                                          -- WinBarNC       xxx guifg=#68545b guibg=#1a1113
    Cursor { bg = "#8b7079", fg = "#1e1517", },                                            -- Cursor         xxx guifg=#1e1517 guibg=#8b7079
    lCursor { Cursor },                                                                    -- lCursor        xxx links to Cursor
    CursorIM { Cursor },                                                                   -- CursorIM       xxx links to Cursor
    HopCursor { Cursor },                                                                  -- HopCursor      xxx links to Cursor
    FloatTitle { gui = "bold", bg = "#1a1113", fg = "#9cbdc9", },                          -- FloatTitle     xxx cterm=bold gui=bold guifg=#9cbdc9 guibg=#1a1113
    FloatFooter { FloatTitle },                                                            -- FloatFooter    xxx links to FloatTitle
    MasonHeader { FloatTitle },                                                            -- MasonHeader    xxx links to FloatTitle
    RedrawDebugNormal { gui = "reverse", },                                                -- RedrawDebugNormal xxx cterm=reverse gui=reverse
    Underlined { gui = "underline", fg = "#26c485", },                                     -- Underlined     xxx cterm=underline gui=underline guifg=#26c485
    Substitute { gui = "bold", bg = "#e77777", fg = "#1e1517", },                          -- Substitute     xxx cterm=bold gui=bold guifg=#1e1517 guibg=#e77777
    VisualNOS { fg = "#1b282c", },                                                         -- VisualNOS      xxx guifg=#1b282c
    Normal { bg = "#1e1517", fg = "#dadada", },                                            -- Normal         xxx guifg=#dadada guibg=#1e1517
    NvimSpacing { Normal },                                                                -- NvimSpacing    xxx links to Normal
    DapUIFrameName { Normal },                                                             -- DapUIFrameName xxx links to Normal
    TelescopeNormal { Normal },                                                            -- TelescopeNormal xxx links to Normal
    Character { fg = "#6d9f71", },                                                         -- Character      xxx guifg=#6d9f71
    sym "@character" { Character },                                                        -- @character     xxx links to Character
    Constant { fg = "#ffd770", },                                                          -- Constant       xxx guifg=#ffd770
    sym "@constant" { Constant },                                                          -- @constant      xxx links to Constant
    luaConstant { Constant },                                                              -- luaConstant    xxx links to Constant
    TelescopePreviewCharDev { Constant },                                                  -- TelescopePreviewCharDev xxx links to Constant
    TelescopePreviewPipe { Constant },                                                     -- TelescopePreviewPipe xxx links to Constant
    TelescopeResultsConstant { Constant },                                                 -- TelescopeResultsConstant xxx links to Constant
    TelescopePreviewGroup { Constant },                                                    -- TelescopePreviewGroup xxx links to Constant
    TelescopePreviewUser { Constant },                                                     -- TelescopePreviewUser xxx links to Constant
    TelescopePreviewRead { Constant },                                                     -- TelescopePreviewRead xxx links to Constant
    TelescopePreviewBlock { Constant },                                                    -- TelescopePreviewBlock xxx links to Constant
    Number { fg = "#ef9474", },                                                            -- Number         xxx guifg=#ef9474
    sym "@number" { Number },                                                              -- @number        xxx links to Number
    NvimNumber { Number },                                                                 -- NvimNumber     xxx links to Number
    DapUILineNumber { Number },                                                            -- DapUILineNumber xxx links to Number
    luaNumber { Number },                                                                  -- luaNumber      xxx links to Number
    TelescopeResultsNumber { Number },                                                     -- TelescopeResultsNumber xxx links to Number
    Boolean { fg = "#ef9474", },                                                           -- Boolean        xxx guifg=#ef9474
    sym "@boolean" { Boolean },                                                            -- @boolean       xxx links to Boolean
    Float { fg = "#ffd770", },                                                             -- Float          xxx guifg=#ffd770
    sym "@number.float" { Float },                                                         -- @number.float  xxx links to Float
    Conditional { fg = "#cd87ba", },                                                       -- Conditional    xxx guifg=#cd87ba
    sym "@keyword.conditional" { Conditional },                                            -- @keyword.conditional xxx links to Conditional
    luaCondElse { Conditional },                                                           -- luaCondElse    xxx links to Conditional
    luaCond { Conditional },                                                               -- luaCond        xxx links to Conditional
    Statement { fg = "#cd87ba", },                                                         -- Statement      xxx guifg=#cd87ba
    luaStatement { Statement },                                                            -- luaStatement   xxx links to Statement
    TelescopePreviewWrite { Statement },                                                   -- TelescopePreviewWrite xxx links to Statement
    TelescopePreviewSocket { Statement },                                                  -- TelescopePreviewSocket xxx links to Statement
    Repeat { fg = "#cd87ba", },                                                            -- Repeat         xxx guifg=#cd87ba
    sym "@keyword.repeat" { Repeat },                                                      -- @keyword.repeat xxx links to Repeat
    luaRepeat { Repeat },                                                                  -- luaRepeat      xxx links to Repeat
    luaFor { Repeat },                                                                     -- luaFor         xxx links to Repeat
    Label { fg = "#4fa9c6", },                                                             -- Label          xxx guifg=#4fa9c6
    sym "@label" { Label },                                                                -- @label         xxx links to Label
    LspInfoBorder { Label },                                                               -- LspInfoBorder  xxx links to Label
    NullLsInfoHeader { Label },                                                            -- NullLsInfoHeader xxx links to Label
    luaLabel { Label },                                                                    -- luaLabel       xxx links to Label
    Keyword { fg = "#cd87ba", },                                                           -- Keyword        xxx guifg=#cd87ba
    sym "@module" { Keyword },                                                             -- @module        xxx links to Keyword
    sym "@keyword" { Keyword },                                                            -- @keyword       xxx links to Keyword
    sym "@keyword.directive" { Keyword },                                                  -- @keyword.directive xxx links to Keyword
    sym "@keyword.directive.define" { Keyword },                                           -- @keyword.directive.define xxx links to Keyword
    DapUISource { Keyword },                                                               -- DapUISource    xxx links to Keyword
    sym "@type.qualifier.typescript" { Keyword },                                          -- @type.qualifier.typescript xxx links to Keyword
    sym "@type.qualifier.php" { Keyword },                                                 -- @type.qualifier.php xxx links to Keyword
    sym "@keyword.return" { Keyword },                                                     -- @keyword.return xxx links to Keyword
    sym "@keyword.operator" { Keyword },                                                   -- @keyword.operator xxx links to Keyword
    sym "@keyword.function" { Keyword },                                                   -- @keyword.function xxx links to Keyword
    sym "@keyword.corotine" { Keyword },                                                   -- @keyword.corotine xxx links to Keyword
    TelescopePreviewSticky { Keyword },                                                    -- TelescopePreviewSticky xxx links to Keyword
    Exception { fg = "#cd87ba", },                                                         -- Exception      xxx guifg=#cd87ba
    sym "@keyword.exception" { Exception },                                                -- @keyword.exception xxx links to Exception
    Include { fg = "#cd87ba", },                                                           -- Include        xxx guifg=#cd87ba
    sym "@keyword.import" { Include },                                                     -- @keyword.import xxx links to Include
    PreProc { fg = "#ffd770", },                                                           -- PreProc        xxx guifg=#ffd770
    Define { fg = "#cd87ba", },                                                            -- Define         xxx guifg=#cd87ba
    Macro { fg = "#ef9474", },                                                             -- Macro          xxx guifg=#ef9474
    PreCondit { fg = "#4fa9c6", },                                                         -- PreCondit      xxx guifg=#4fa9c6
    StorageClass { fg = "#4fa9c6", },                                                      -- StorageClass   xxx guifg=#4fa9c6
    sym "@keyword.storage" { StorageClass },                                               -- @keyword.storage xxx links to StorageClass
    Type { fg = "#4fa9c6", },                                                              -- Type           xxx guifg=#4fa9c6
    sym "@type" { Type },                                                                  -- @type          xxx links to Type
    NvimNumberPrefix { Type },                                                             -- NvimNumberPrefix xxx links to Type
    NvimOptionSigil { Type },                                                              -- NvimOptionSigil xxx links to Type
    DapUIType { Type },                                                                    -- DapUIType      xxx links to Type
    LspInfoFiletype { Type },                                                              -- LspInfoFiletype xxx links to Type
    NullLsInfoTitle { Type },                                                              -- NullLsInfoTitle xxx links to Type
    Structure { fg = "#ffd770", },                                                         -- Structure      xxx guifg=#ffd770
    sym "@keyword.modifier" { Structure },                                                 -- @keyword.modifier xxx links to Structure
    luaTable { Structure },                                                                -- luaTable       xxx links to Structure
    Typedef { fg = "#ffd770", },                                                           -- Typedef        xxx guifg=#ffd770
    sym "@keyword.type" { Typedef },                                                       -- @keyword.type  xxx links to Typedef
    Tag { fg = "#4fa9c6", },                                                               -- Tag            xxx guifg=#4fa9c6
    Special { fg = "#4fa9c6", },                                                           -- Special        xxx guifg=#4fa9c6
    sym "@markup" { Special },                                                             -- @markup        xxx links to Special
    sym "@tag.builtin" { Special },                                                        -- @tag.builtin   xxx links to Special
    sym "@markup.list" { Special },                                                        -- @markup.list   xxx links to Special
    TelescopePreviewLink { Special },                                                      -- TelescopePreviewLink xxx links to Special
    SpecialChar { fg = "#4fa9c6", },                                                       -- SpecialChar    xxx guifg=#4fa9c6
    sym "@character.special" { SpecialChar },                                              -- @character.special xxx links to SpecialChar
    NvimRegister { SpecialChar },                                                          -- NvimRegister   xxx links to SpecialChar
    NvimStringSpecial { SpecialChar },                                                     -- NvimStringSpecial xxx links to SpecialChar
    luaSpecial { SpecialChar },                                                            -- luaSpecial     xxx links to SpecialChar
    NeoTreeBufferNumber { SpecialChar },                                                   -- NeoTreeBufferNumber xxx links to SpecialChar
    NeoTreeFilterTerm { SpecialChar },                                                     -- NeoTreeFilterTerm xxx links to SpecialChar
    TelescopeResultsVariable { SpecialChar },                                              -- TelescopeResultsVariable xxx links to SpecialChar
    SpecialComment { fg = "#393337", },                                                    -- SpecialComment xxx guifg=#393337
    TelescopeResultsSpecialComment { SpecialComment },                                     -- TelescopeResultsSpecialComment xxx links to SpecialComment
    Debug { fg = "#9cbdc9", },                                                             -- Debug          xxx guifg=#9cbdc9
    sym "@keyword.debug" { Debug },                                                        -- @keyword.debug xxx links to Debug
    LspCodeLens { fg = "#393337", },                                                       -- LspCodeLens    xxx guifg=#393337
    LspCodeLensSeparator { fg = "#393337", },                                              -- LspCodeLensSeparator xxx guifg=#393337
    LspReferenceRead { bg = "#251d1f", },                                                  -- LspReferenceRead xxx guibg=#251d1f
    LspReferenceText { LspReferenceRead },                                                 -- LspReferenceText xxx links to LspReferenceRead
    LspReferenceWrite { LspReferenceRead },                                                -- LspReferenceWrite xxx links to LspReferenceRead
    IlluminatedWordRead { LspReferenceRead },                                              -- IlluminatedWordRead xxx links to LspReferenceRead
    DiagnosticError { fg = "#e77777", },                                                   -- DiagnosticError xxx guifg=#e77777
    DiagnosticFloatingError { DiagnosticError },                                           -- DiagnosticFloatingError xxx links to DiagnosticError
    DiagnosticVirtualTextError { DiagnosticError },                                        -- DiagnosticVirtualTextError xxx links to DiagnosticError
    DiagnosticSignError { DiagnosticError },                                               -- DiagnosticSignError xxx links to DiagnosticError
    DiagnosticWarn { fg = "#ffd770", },                                                    -- DiagnosticWarn xxx guifg=#ffd770
    DiagnosticFloatingWarn { DiagnosticWarn },                                             -- DiagnosticFloatingWarn xxx links to DiagnosticWarn
    DiagnosticVirtualTextWarn { DiagnosticWarn },                                          -- DiagnosticVirtualTextWarn xxx links to DiagnosticWarn
    DiagnosticSignWarn { DiagnosticWarn },                                                 -- DiagnosticSignWarn xxx links to DiagnosticWarn
    LazyNoCond { DiagnosticWarn },                                                         -- LazyNoCond     xxx links to DiagnosticWarn
    DiagnosticInfo { fg = "#9cbdc9", },                                                    -- DiagnosticInfo xxx guifg=#9cbdc9
    DiagnosticFloatingInfo { DiagnosticInfo },                                             -- DiagnosticFloatingInfo xxx links to DiagnosticInfo
    DiagnosticVirtualTextInfo { DiagnosticInfo },                                          -- DiagnosticVirtualTextInfo xxx links to DiagnosticInfo
    DiagnosticSignInfo { DiagnosticInfo },                                                 -- DiagnosticSignInfo xxx links to DiagnosticInfo
    sym "@comment.note" { DiagnosticInfo },                                                -- @comment.note  xxx links to DiagnosticInfo
    DiagnosticHint { fg = "#88c3ab", },                                                    -- DiagnosticHint xxx guifg=#88c3ab
    DiagnosticFloatingHint { DiagnosticHint },                                             -- DiagnosticFloatingHint xxx links to DiagnosticHint
    DiagnosticVirtualTextHint { DiagnosticHint },                                          -- DiagnosticVirtualTextHint xxx links to DiagnosticHint
    DiagnosticSignHint { DiagnosticHint },                                                 -- DiagnosticSignHint xxx links to DiagnosticHint
    DiagnosticOk { fg = "#9ac374", },                                                      -- DiagnosticOk   xxx guifg=#9ac374
    DiagnosticFloatingOk { DiagnosticOk },                                                 -- DiagnosticFloatingOk xxx links to DiagnosticOk
    DiagnosticVirtualTextOk { DiagnosticOk },                                              -- DiagnosticVirtualTextOk xxx links to DiagnosticOk
    DiagnosticSignOk { DiagnosticOk },                                                     -- DiagnosticSignOk xxx links to DiagnosticOk
    Comment { gui = "italic", fg = "#63646a", },                                           -- Comment        xxx cterm=italic gui=italic guifg=#63646a
    DiagnosticUnnecessary { Comment },                                                     -- DiagnosticUnnecessary xxx links to Comment
    sym "@comment" { Comment },                                                            -- @comment       xxx links to Comment
    LazyComment { Comment },                                                               -- LazyComment    xxx links to Comment
    DapUIBreakpointsDisabledLine { Comment },                                              -- DapUIBreakpointsDisabledLine xxx links to Comment
    UfoFoldedEllipsis { Comment },                                                         -- UfoFoldedEllipsis xxx links to Comment
    LspInfoTip { Comment },                                                                -- LspInfoTip     xxx links to Comment
    luaComment { Comment },                                                                -- luaComment     xxx links to Comment
    AerialPrivate { Comment },                                                             -- AerialPrivate  xxx links to Comment
    AerialProtected { Comment },                                                           -- AerialProtected xxx links to Comment
    TelescopeResultsComment { Comment },                                                   -- TelescopeResultsComment xxx links to Comment
    sym "@variable.builtin" { fg = "#26c485", },                                           -- @variable.builtin xxx guifg=#26c485
    sym "@lsp.typemod.variable.defaultLibrary" { sym "@variable.builtin" },                -- @lsp.typemod.variable.defaultLibrary xxx links to @variable.builtin
    sym "@lsp.type.selfKeyword" { sym "@variable.builtin" },                               -- @lsp.type.selfKeyword xxx links to @variable.builtin
    sym "@variable.parameter" { fg = "#ef9474", },                                         -- @variable.parameter xxx guifg=#ef9474
    sym "@variable.parameter.builtin" { sym "@variable.parameter" },                       -- @variable.parameter.builtin xxx links to @variable.parameter
    sym "@parameter" { sym "@variable.parameter" },                                        -- @parameter     xxx links to @variable.parameter
    sym "@module.builtin" { fg = "#ffd770", },                                             -- @module.builtin xxx guifg=#ffd770
    sym "@namespace.builtin" { sym "@module.builtin" },                                    -- @namespace.builtin xxx links to @module.builtin
    String { fg = "#6d9f71", },                                                            -- String         xxx guifg=#6d9f71
    sym "@string" { String },                                                              -- @string        xxx links to String
    sym "@string.regexp" { String },                                                       -- @string.regexp xxx links to String
    sym "@string.special" { String },                                                      -- @string.special xxx links to String
    NvimString { String },                                                                 -- NvimString     xxx links to String
    sym "@string.special.symbol" { String },                                               -- @string.special.symbol xxx links to String
    DapUIThread { String },                                                                -- DapUIThread    xxx links to String
    sym "@markup.link.label" { String },                                                   -- @markup.link.label xxx links to String
    sym "@string.special.path" { String },                                                 -- @string.special.path xxx links to String
    sym "@string.documentation" { String },                                                -- @string.documentation xxx links to String
    luaString2 { String },                                                                 -- luaString2     xxx links to String
    luaString { String },                                                                  -- luaString      xxx links to String
    TelescopePreviewSize { String },                                                       -- TelescopePreviewSize xxx links to String
    TelescopePreviewExecute { String },                                                    -- TelescopePreviewExecute xxx links to String
    sym "@string.escape" { fg = "#df8489", },                                              -- @string.escape xxx guifg=#df8489
    sym "@lsp.type.escapeSequence" { sym "@string.escape" },                               -- @lsp.type.escapeSequence xxx links to @string.escape
    sym "@type.builtin" { fg = "#ffd770", },                                               -- @type.builtin  xxx guifg=#ffd770
    sym "@lsp.typemod.class.defaultLibrary" { sym "@type.builtin" },                       -- @lsp.typemod.class.defaultLibrary xxx links to @type.builtin
    sym "@lsp.type.builtinType" { sym "@type.builtin" },                                   -- @lsp.type.builtinType xxx links to @type.builtin
    sym "@lsp.typemod.type.defaultLibrary" { sym "@type.builtin" },                        -- @lsp.typemod.type.defaultLibrary xxx links to @type.builtin
    sym "@lsp.typemod.enum.defaultLibrary" { sym "@type.builtin" },                        -- @lsp.typemod.enum.defaultLibrary xxx links to @type.builtin
    sym "@attribute" { fg = "#ffd770", },                                                  -- @attribute     xxx guifg=#ffd770
    sym "@attribute.builtin" { sym "@attribute" },                                         -- @attribute.builtin xxx links to @attribute
    sym "@property" { fg = "#df8489", },                                                   -- @property      xxx guifg=#df8489
    sym "@lsp.type.property" { sym "@property" },                                          -- @lsp.type.property xxx links to @property
    AerialPropertyIcon { sym "@property" },                                                -- AerialPropertyIcon xxx links to @property
    CmpItemKindProperty { sym "@property" },                                               -- CmpItemKindProperty xxx links to @property
    sym "@variable.member" { sym "@property" },                                            -- @variable.member xxx links to @property
    Identifier { fg = "#dadada", },                                                        -- Identifier     xxx guifg=#dadada
    sym "@variable" { Identifier },                                                        -- @variable      xxx links to Identifier
    NvimIdentifier { Identifier },                                                         -- NvimIdentifier xxx links to Identifier
    DapUIVariable { Identifier },                                                          -- DapUIVariable  xxx links to Identifier
    luaFunc { Identifier },                                                                -- luaFunc        xxx links to Identifier
    TelescopeResultsIdentifier { Identifier },                                             -- TelescopeResultsIdentifier xxx links to Identifier
    Function { fg = "#4fa9c6", },                                                          -- Function       xxx guifg=#4fa9c6
    sym "@function" { Function },                                                          -- @function      xxx links to Function
    sym "@function.method.call.php" { Function },                                          -- @function.method.call.php xxx links to Function
    sym "@function.method.php" { Function },                                               -- @function.method.php xxx links to Function
    LspInfoList { Function },                                                              -- LspInfoList    xxx links to Function
    luaMetaMethod { Function },                                                            -- luaMetaMethod  xxx links to Function
    luaFunction { Function },                                                              -- luaFunction    xxx links to Function
    TelescopeResultsFunction { Function },                                                 -- TelescopeResultsFunction xxx links to Function
    TelescopeResultsField { Function },                                                    -- TelescopeResultsField xxx links to Function
    TelescopeResultsClass { Function },                                                    -- TelescopeResultsClass xxx links to Function
    sym "@function.builtin" { fg = "#26c485", },                                           -- @function.builtin xxx guifg=#26c485
    sym "@lsp.typemod.function.global" { sym "@function.builtin" },                        -- @lsp.typemod.function.global xxx links to @function.builtin
    sym "@lsp.typemod.function.defaultLibrary" { sym "@function.builtin" },                -- @lsp.typemod.function.defaultLibrary xxx links to @function.builtin
    sym "@lsp.typemod.method.defaultLibrary" { sym "@function.builtin" },                  -- @lsp.typemod.method.defaultLibrary xxx links to @function.builtin
    sym "@lsp.typemod.macro.defaultLibrary" { sym "@function.builtin" },                   -- @lsp.typemod.macro.defaultLibrary xxx links to @function.builtin
    Operator { fg = "#dadada", },                                                          -- Operator       xxx guifg=#dadada
    sym "@operator" { Operator },                                                          -- @operator      xxx links to Operator
    NvimAssignment { Operator },                                                           -- NvimAssignment xxx links to Operator
    NvimOperator { Operator },                                                             -- NvimOperator   xxx links to Operator
    luaOperator { Operator },                                                              -- luaOperator    xxx links to Operator
    TelescopeResultsOperator { Operator },                                                 -- TelescopeResultsOperator xxx links to Operator
    Delimiter { fg = "#4fa9c6", },                                                         -- Delimiter      xxx guifg=#4fa9c6
    sym "@punctuation" { Delimiter },                                                      -- @punctuation   xxx links to Delimiter
    NvimParenthesis { Delimiter },                                                         -- NvimParenthesis xxx links to Delimiter
    NvimColon { Delimiter },                                                               -- NvimColon      xxx links to Delimiter
    NvimComma { Delimiter },                                                               -- NvimComma      xxx links to Delimiter
    NvimArrow { Delimiter },                                                               -- NvimArrow      xxx links to Delimiter
    sym "@punctuation.special" { fg = "#dadada", },                                        -- @punctuation.special xxx guifg=#dadada
    sym "@punctuation.special.markdown" { sym "@punctuation.special" },                    -- @punctuation.special.markdown xxx links to @punctuation.special
    sym "@lsp.type.formatSpecifier" { sym "@punctuation.special" },                        -- @lsp.type.formatSpecifier xxx links to @punctuation.special
    sym "@comment.error" { bg = "#e77777", fg = "#1e1517", },                              -- @comment.error xxx guifg=#1e1517 guibg=#e77777
    sym "@text.danger" { sym "@comment.error" },                                           -- @text.danger   xxx links to @comment.error
    sym "@comment.warning" { bg = "#faa27f", fg = "#1e1517", },                            -- @comment.warning xxx guifg=#1e1517 guibg=#faa27f
    sym "@text.warning" { sym "@comment.warning" },                                        -- @text.warning  xxx links to @comment.warning
    sym "@comment.todo" { bg = "#ffd770", fg = "#1e1517", },                               -- @comment.todo  xxx guifg=#1e1517 guibg=#ffd770
    sym "@text.todo" { sym "@comment.todo" },                                              -- @text.todo     xxx links to @comment.todo
    Todo { fg = "#ffd770", },                                                              -- Todo           xxx guifg=#ffd770
    luaTodo { Todo },                                                                      -- luaTodo        xxx links to Todo
    sym "@markup.strong" { gui = "bold", fg = "#dadada", },                                -- @markup.strong xxx cterm=bold gui=bold guifg=#dadada
    sym "@text.strong" { sym "@markup.strong" },                                           -- @text.strong   xxx links to @markup.strong
    sym "@markup.italic" { gui = "italic", fg = "#dadada", },                              -- @markup.italic xxx cterm=italic gui=italic guifg=#dadada
    sym "@markup.strikethrough" { gui = "strikethrough", fg = "#dadada", },                -- @markup.strikethrough xxx cterm=strikethrough gui=strikethrough guifg=#dadada
    sym "@text.strike" { sym "@markup.strikethrough" },                                    -- @text.strike   xxx links to @markup.strikethrough
    sym "@markup.heading" { gui = "bold", fg = "#dadada", },                               -- @markup.heading xxx cterm=bold gui=bold guifg=#dadada
    sym "@text.title" { sym "@markup.heading" },                                           -- @text.title    xxx links to @markup.heading
    sym "@markup.link" { gui = "bold", fg = "#ffd770", },                                  -- @markup.link   xxx cterm=bold gui=bold guifg=#ffd770
    sym "@text.reference" { sym "@markup.link" },                                          -- @text.reference xxx links to @markup.link
    Added { fg = "nvimlightgreen", },                                                      -- Added          xxx ctermfg=10 guifg=NvimLightGreen
    Removed { fg = "nvimlightred", },                                                      -- Removed        xxx ctermfg=9 guifg=NvimLightRed
    Changed { fg = "nvimlightcyan", },                                                     -- Changed        xxx ctermfg=14 guifg=NvimLightCyan
    sym "@tag" { fg = "#df8489", },                                                        -- @tag           xxx guifg=#df8489
    DiagnosticDeprecated { gui = "strikethrough", sp = "nvimlightred", },                  -- DiagnosticDeprecated xxx cterm=strikethrough gui=strikethrough guisp=NvimLightRed
    sym "@lsp.mod.deprecated" { DiagnosticDeprecated },                                    -- @lsp.mod.deprecated xxx links to DiagnosticDeprecated
    FloatShadow { bg = "nvimdarkgrey4", blend = 80, },                                     -- FloatShadow    xxx ctermbg=0 guibg=NvimDarkGrey4 blend=80
    FloatShadowThrough { bg = "nvimdarkgrey4", blend = 100, },                             -- FloatShadowThrough xxx ctermbg=0 guibg=NvimDarkGrey4 blend=100
    MatchParen { gui = "bold", fg = "#faa27f", },                                          -- MatchParen     xxx cterm=bold gui=bold guifg=#faa27f
    RedrawDebugClear { bg = "nvimdarkyellow", },                                           -- RedrawDebugClear xxx ctermfg=0 ctermbg=11 guibg=NvimDarkYellow
    RedrawDebugComposed { bg = "nvimdarkgreen", },                                         -- RedrawDebugComposed xxx ctermfg=0 ctermbg=10 guibg=NvimDarkGreen
    RedrawDebugRecompose { bg = "nvimdarkred", },                                          -- RedrawDebugRecompose xxx ctermfg=0 ctermbg=9 guibg=NvimDarkRed
    Error { fg = "#e77777", },                                                             -- Error          xxx guifg=#e77777
    NvimInvalid { Error },                                                                 -- NvimInvalid    xxx links to Error
    sym "@error" { Error },                                                                -- @error         xxx links to Error
    luaParenError { Error },                                                               -- luaParenError  xxx links to Error
    luaError { Error },                                                                    -- luaError       xxx links to Error
    DiagnosticUnderlineError { gui = "undercurl", sp = "#e77777", },                       -- DiagnosticUnderlineError xxx cterm=undercurl gui=undercurl guisp=#e77777
    DiagnosticUnderlineWarn { gui = "undercurl", sp = "#ffd770", },                        -- DiagnosticUnderlineWarn xxx cterm=undercurl gui=undercurl guisp=#ffd770
    DiagnosticUnderlineInfo { gui = "undercurl", sp = "#9cbdc9", },                        -- DiagnosticUnderlineInfo xxx cterm=undercurl gui=undercurl guisp=#9cbdc9
    DiagnosticUnderlineHint { gui = "undercurl", sp = "#88c3ab", },                        -- DiagnosticUnderlineHint xxx cterm=undercurl gui=undercurl guisp=#88c3ab
    DiagnosticUnderlineOk { gui = "undercurl", sp = "#9ac374", },                          -- DiagnosticUnderlineOk xxx cterm=undercurl gui=undercurl guisp=#9ac374
    NvimInternalError { bg = "red", fg = "red", },                                         -- NvimInternalError xxx ctermfg=9 ctermbg=9 guifg=Red guibg=Red
    NvimFigureBrace { NvimInternalError },                                                 -- NvimFigureBrace xxx links to NvimInternalError
    NvimSingleQuotedUnknownEscape { NvimInternalError },                                   -- NvimSingleQuotedUnknownEscape xxx links to NvimInternalError
    NvimInvalidSingleQuotedUnknownEscape { NvimInternalError },                            -- NvimInvalidSingleQuotedUnknownEscape xxx links to NvimInternalError
    HighlightURL { gui = "underline", },                                                   -- HighlightURL   xxx cterm=underline gui=underline
    TelescopeMultiIcon { fg = "#9cbdc9", },                                                -- TelescopeMultiIcon xxx guifg=#9cbdc9
    TelescopeMultiSelection { fg = "#4fa9c6", },                                           -- TelescopeMultiSelection xxx guifg=#4fa9c6
    TelescopeMatching { gui = "bold", fg = "#ffeeee", },                                   -- TelescopeMatching xxx cterm=bold gui=bold guifg=#ffeeee
    TelescopeSelectionCaret { fg = "#9cbdc9", },                                           -- TelescopeSelectionCaret xxx guifg=#9cbdc9
    TelescopePromptPrefix { fg = "#9cbdc9", },                                             -- TelescopePromptPrefix xxx guifg=#9cbdc9
    TelescopePreviewBorder { bg = "#1e1517", fg = "#736468", },                            -- TelescopePreviewBorder xxx guifg=#736468 guibg=#1e1517
    TelescopeResultsBorder { bg = "#1e1517", fg = "#736468", },                            -- TelescopeResultsBorder xxx guifg=#736468 guibg=#1e1517
    TelescopePromptBorder { bg = "#1e1517", fg = "#736468", },                             -- TelescopePromptBorder xxx guifg=#736468 guibg=#1e1517
    TelescopePreviewNormal { bg = "#1e1517", fg = "#8b7079", },                            -- TelescopePreviewNormal xxx guifg=#8b7079 guibg=#1e1517
    TelescopePreviewMessage { TelescopePreviewNormal },                                    -- TelescopePreviewMessage xxx links to TelescopePreviewNormal
    TelescopeResultsNormal { bg = "#1e1517", fg = "#8b7079", },                            -- TelescopeResultsNormal xxx guifg=#8b7079 guibg=#1e1517
    TelescopePromptNormal { bg = "#1e1517", fg = "#8b7079", },                             -- TelescopePromptNormal xxx guifg=#8b7079 guibg=#1e1517
    TelescopePreviewTitle { gui = "bold", bg = "#1e1517", fg = "#9cbdc9", },               -- TelescopePreviewTitle xxx cterm=bold gui=bold guifg=#9cbdc9 guibg=#1e1517
    TelescopeResultsTitle { gui = "bold", bg = "#1e1517", fg = "#9cbdc9", },               -- TelescopeResultsTitle xxx cterm=bold gui=bold guifg=#9cbdc9 guibg=#1e1517
    TelescopePromptTitle { gui = "bold", bg = "#281e22", fg = "#9cbdc9", },                -- TelescopePromptTitle xxx cterm=bold gui=bold guifg=#9cbdc9 guibg=#281e22
    LazyCommitScope { gui = "italic", },                                                   -- LazyCommitScope xxx cterm=italic gui=italic
    LazyCommitType { gui = "bold", fg = "#9cbdc9", },                                      -- LazyCommitType xxx cterm=bold gui=bold guifg=#9cbdc9
    LazyCommitIssue { fg = "#ffd770", },                                                   -- LazyCommitIssue xxx guifg=#ffd770
    LazyCommit { gui = "bold", fg = "#68545b", },                                          -- LazyCommit     xxx cterm=bold gui=bold guifg=#68545b
    LazyH2 { gui = "bold", fg = "#9cbdc9", },                                              -- LazyH2         xxx cterm=bold gui=bold guifg=#9cbdc9
    LazyH1 { gui = "bold", bg = "#9cbdc9", fg = "#1e1517", },                              -- LazyH1         xxx cterm=bold gui=bold guifg=#1e1517 guibg=#9cbdc9
    NotifyLogTitle { bg = "#1a1113", fg = "#9cbdc9", },                                    -- NotifyLogTitle xxx guifg=#9cbdc9 guibg=#1a1113
    NotifyLogTime { bg = "#1a1113", fg = "#68545b", },                                     -- NotifyLogTime  xxx guifg=#68545b guibg=#1a1113
    NotifyTRACEBody { bg = "#1a1113", fg = "#8b7079", },                                   -- NotifyTRACEBody xxx guifg=#8b7079 guibg=#1a1113
    NotifyDEBUGBody { bg = "#1a1113", fg = "#8b7079", },                                   -- NotifyDEBUGBody xxx guifg=#8b7079 guibg=#1a1113
    NotifyINFOBody { bg = "#1a1113", fg = "#8b7079", },                                    -- NotifyINFOBody xxx guifg=#8b7079 guibg=#1a1113
    NotifyWARNBody { bg = "#1a1113", fg = "#8b7079", },                                    -- NotifyWARNBody xxx guifg=#8b7079 guibg=#1a1113
    NotifyERRORBody { bg = "#1a1113", fg = "#8b7079", },                                   -- NotifyERRORBody xxx guifg=#8b7079 guibg=#1a1113
    NotifyTRACETitle { bg = "#1a1113", fg = "#efbde3", },                                  -- NotifyTRACETitle xxx guifg=#efbde3 guibg=#1a1113
    NotifyDEBUGTitle { bg = "#1a1113", fg = "#88c3ab", },                                  -- NotifyDEBUGTitle xxx guifg=#88c3ab guibg=#1a1113
    NotifyINFOTitle { bg = "#1a1113", fg = "#9ac374", },                                   -- NotifyINFOTitle xxx guifg=#9ac374 guibg=#1a1113
    NotifyWARNTitle { bg = "#1a1113", fg = "#faa27f", },                                   -- NotifyWARNTitle xxx guifg=#faa27f guibg=#1a1113
    NotifyERRORTitle { bg = "#1a1113", fg = "#e77777", },                                  -- NotifyERRORTitle xxx guifg=#e77777 guibg=#1a1113
    NotifyTRACEIcon { bg = "#1a1113", fg = "#efbde3", },                                   -- NotifyTRACEIcon xxx guifg=#efbde3 guibg=#1a1113
    NotifyDEBUGIcon { bg = "#1a1113", fg = "#88c3ab", },                                   -- NotifyDEBUGIcon xxx guifg=#88c3ab guibg=#1a1113
    NotifyINFOIcon { bg = "#1a1113", fg = "#9ac374", },                                    -- NotifyINFOIcon xxx guifg=#9ac374 guibg=#1a1113
    NotifyWARNIcon { bg = "#1a1113", fg = "#faa27f", },                                    -- NotifyWARNIcon xxx guifg=#faa27f guibg=#1a1113
    NotifyERRORIcon { bg = "#1a1113", fg = "#e77777", },                                   -- NotifyERRORIcon xxx guifg=#e77777 guibg=#1a1113
    NotifyTRACERBorder { bg = "#1a1113", fg = "#efbde3", },                                -- NotifyTRACERBorder xxx guifg=#efbde3 guibg=#1a1113
    NotifyDEBUGBorder { bg = "#1a1113", fg = "#88c3ab", },                                 -- NotifyDEBUGBorder xxx guifg=#88c3ab guibg=#1a1113
    NotifyINFOBorder { bg = "#1a1113", fg = "#9ac374", },                                  -- NotifyINFOBorder xxx guifg=#9ac374 guibg=#1a1113
    NotifyWARNBorder { bg = "#1a1113", fg = "#faa27f", },                                  -- NotifyWARNBorder xxx guifg=#faa27f guibg=#1a1113
    NotifyERRORBorder { bg = "#1a1113", fg = "#e77777", },                                 -- NotifyERRORBorder xxx guifg=#e77777 guibg=#1a1113
    NotifyBackground { bg = "#1e1517", fg = "#8b7079", },                                  -- NotifyBackground xxx guifg=#8b7079 guibg=#1e1517
    HopUnmatched { fg = "#444145", },                                                      -- HopUnmatched   xxx guifg=#444145
    HopNextKey2 { fg = "#9cbdc9", },                                                       -- HopNextKey2    xxx guifg=#9cbdc9
    HopNextKey1 { gui = "bold", fg = "#88c3ab", },                                         -- HopNextKey1    xxx cterm=bold gui=bold guifg=#88c3ab
    HopNextKey { gui = "bold", fg = "#e77777", },                                          -- HopNextKey     xxx cterm=bold gui=bold guifg=#e77777
    StatusCommand { bg = "#ffd770", fg = "#281e22", },                                     -- StatusCommand  xxx guifg=#281e22 guibg=#ffd770
    HeirlineCommand { StatusCommand },                                                     -- HeirlineCommand xxx links to StatusCommand
    StatusReplace { bg = "#e77777", fg = "#281e22", },                                     -- StatusReplace  xxx guifg=#281e22 guibg=#e77777
    HeirlineReplace { StatusReplace },                                                     -- HeirlineReplace xxx links to StatusReplace
    StatusVisual { bg = "#efbde3", fg = "#281e22", },                                      -- StatusVisual   xxx guifg=#281e22 guibg=#efbde3
    HeirlineVisual { StatusVisual },                                                       -- HeirlineVisual xxx links to StatusVisual
    WindowPickerWinBarNC { fg = "#e77777", },                                              -- WindowPickerWinBarNC xxx guifg=#e77777
    WindowPickerWinBar { fg = "#e77777", },                                                -- WindowPickerWinBar xxx guifg=#e77777
    WindowPickerStatusLineNC { fg = "#e77777", },                                          -- WindowPickerStatusLineNC xxx guifg=#e77777
    WindowPickerStatusLine { fg = "#e77777", },                                            -- WindowPickerStatusLine xxx guifg=#e77777
    WhichKeyGroup { fg = "#9cbdc9", },                                                     -- WhichKeyGroup  xxx guifg=#9cbdc9
    WhichKeyDesc { fg = "#efbde3", },                                                      -- WhichKeyDesc   xxx guifg=#efbde3
    WhichKeySeparator { fg = "#393337", },                                                 -- WhichKeySeparator xxx guifg=#393337
    WhichKey { fg = "#88c3ab", },                                                          -- WhichKey       xxx guifg=#88c3ab
    MasonHeading { gui = "bold", },                                                        -- MasonHeading   xxx cterm=bold gui=bold
    MasonMutedBlockBold { gui = "bold", bg = "#9cbdc9", fg = "#1e1517", },                 -- MasonMutedBlockBold xxx cterm=bold gui=bold guifg=#1e1517 guibg=#9cbdc9
    MasonMuted { fg = "#e77777", },                                                        -- MasonMuted     xxx guifg=#e77777
    MasonHighlight { fg = "#9cbdc9", },                                                    -- MasonHighlight xxx guifg=#9cbdc9
    MasonLink { MasonHighlight },                                                          -- MasonLink      xxx links to MasonHighlight
    MasonHighlightBlockBoldSecondary { gui = "bold", bg = "#9cbdc9", fg = "#1e1517", },    -- MasonHighlightBlockBoldSecondary xxx cterm=bold gui=bold guifg=#1e1517 guibg=#9cbdc9
    MasonHighlightBlockSecondary { bg = "#9cbdc9", fg = "#1e1517", },                      -- MasonHighlightBlockSecondary xxx guifg=#1e1517 guibg=#9cbdc9
    MasonHighlightSecondary { fg = "#9cbdc9", },                                           -- MasonHighlightSecondary xxx guifg=#9cbdc9
    MasonHighlightBlockBold { gui = "bold", bg = "#9cbdc9", fg = "#1e1517", },             -- MasonHighlightBlockBold xxx cterm=bold gui=bold guifg=#1e1517 guibg=#9cbdc9
    MasonHighlightBlock { bg = "#9cbdc9", fg = "#1e1517", },                               -- MasonHighlightBlock xxx guifg=#1e1517 guibg=#9cbdc9
    MasonHeaderSecondary { gui = "bold", bg = "#9cbdc9", fg = "#1e1517", },                -- MasonHeaderSecondary xxx cterm=bold gui=bold guifg=#1e1517 guibg=#9cbdc9
    AerialLine { fg = "#9cbdc9", },                                                        -- AerialLine     xxx guifg=#9cbdc9
    AerialLineNC { AerialLine },                                                           -- AerialLineNC   xxx links to AerialLine
    AerialGuide { fg = "#393337", },                                                       -- AerialGuide    xxx guifg=#393337
    AerialGuide1 { AerialGuide },                                                          -- AerialGuide1   xxx links to AerialGuide
    AerialGuide2 { AerialGuide },                                                          -- AerialGuide2   xxx links to AerialGuide
    AerialGuide3 { AerialGuide },                                                          -- AerialGuide3   xxx links to AerialGuide
    AerialGuide4 { AerialGuide },                                                          -- AerialGuide4   xxx links to AerialGuide
    AerialGuide5 { AerialGuide },                                                          -- AerialGuide5   xxx links to AerialGuide
    AerialGuide6 { AerialGuide },                                                          -- AerialGuide6   xxx links to AerialGuide
    AerialGuide7 { AerialGuide },                                                          -- AerialGuide7   xxx links to AerialGuide
    AerialGuide8 { AerialGuide },                                                          -- AerialGuide8   xxx links to AerialGuide
    AerialGuide9 { AerialGuide },                                                          -- AerialGuide9   xxx links to AerialGuide
    sym "@text" { fg = "#dadada", },                                                       -- @text          xxx guifg=#dadada
    CmpItemKindText { sym "@text" },                                                       -- CmpItemKindText xxx links to @text
    IblWhitespace { fg = "#393337", },                                                     -- IblWhitespace  xxx guifg=#393337
    IndentBlanklineSpaceshar { fg = "#393337", },                                          -- IndentBlanklineSpaceshar xxx guifg=#393337
    IndentBlanklineContextChar { fg = "#8b7079", },                                        -- IndentBlanklineContextChar xxx guifg=#8b7079
    IndentBlanklineContextStart { gui = "underline", fg = "#dadada", },                    -- IndentBlanklineContextStart xxx cterm=underline gui=underline guifg=#dadada
    IndentBlanklineChar { fg = "#393337", },                                               -- IndentBlanklineChar xxx guifg=#393337
    TodoBgBUG { gui = "bold", bg = "#e77777", fg = "#1e1517", },                           -- TodoBgBUG      xxx cterm=bold gui=bold guifg=#1e1517 guibg=#e77777
    TodoBgHACK { gui = "bold", bg = "#ffd770", fg = "#1e1517", },                          -- TodoBgHACK     xxx cterm=bold gui=bold guifg=#1e1517 guibg=#ffd770
    TodoBgWARNING { gui = "bold", bg = "#ffd770", fg = "#1e1517", },                       -- TodoBgWARNING  xxx cterm=bold gui=bold guifg=#1e1517 guibg=#ffd770
    TodoBgTODO { gui = "bold", bg = "#9cbdc9", fg = "#1e1517", },                          -- TodoBgTODO     xxx cterm=bold gui=bold guifg=#1e1517 guibg=#9cbdc9
    TodoBgNOTE { gui = "bold", bg = "#88c3ab", fg = "#1e1517", },                          -- TodoBgNOTE     xxx cterm=bold gui=bold guifg=#1e1517 guibg=#88c3ab
    CmpItemKind { fg = "#ffd770", },                                                       -- CmpItemKind    xxx guifg=#ffd770
    CmpItemKindConstantDefault { CmpItemKind },                                            -- CmpItemKindConstantDefault xxx links to CmpItemKind
    CmpItemKindPropertyDefault { CmpItemKind },                                            -- CmpItemKindPropertyDefault xxx links to CmpItemKind
    CmpItemKindTypeParameterDefault { CmpItemKind },                                       -- CmpItemKindTypeParameterDefault xxx links to CmpItemKind
    CmpItemKindReferenceDefault { CmpItemKind },                                           -- CmpItemKindReferenceDefault xxx links to CmpItemKind
    CmpItemKindColorDefault { CmpItemKind },                                               -- CmpItemKindColorDefault xxx links to CmpItemKind
    CmpItemKindFileDefault { CmpItemKind },                                                -- CmpItemKindFileDefault xxx links to CmpItemKind
    CmpItemKindTextDefault { CmpItemKind },                                                -- CmpItemKindTextDefault xxx links to CmpItemKind
    CmpItemKindSnippetDefault { CmpItemKind },                                             -- CmpItemKindSnippetDefault xxx links to CmpItemKind
    CmpItemKindKeywordDefault { CmpItemKind },                                             -- CmpItemKindKeywordDefault xxx links to CmpItemKind
    CmpItemKindClassDefault { CmpItemKind },                                               -- CmpItemKindClassDefault xxx links to CmpItemKind
    CmpItemKindFunctionDefault { CmpItemKind },                                            -- CmpItemKindFunctionDefault xxx links to CmpItemKind
    CmpItemKindVariableDefault { CmpItemKind },                                            -- CmpItemKindVariableDefault xxx links to CmpItemKind
    CmpItemKindConstructorDefault { CmpItemKind },                                         -- CmpItemKindConstructorDefault xxx links to CmpItemKind
    CmpItemKindStructDefault { CmpItemKind },                                              -- CmpItemKindStructDefault xxx links to CmpItemKind
    CmpItemKindModuleDefault { CmpItemKind },                                              -- CmpItemKindModuleDefault xxx links to CmpItemKind
    CmpItemKindMethodDefault { CmpItemKind },                                              -- CmpItemKindMethodDefault xxx links to CmpItemKind
    CmpItemKindInterfaceDefault { CmpItemKind },                                           -- CmpItemKindInterfaceDefault xxx links to CmpItemKind
    CmpItemKindFolderDefault { CmpItemKind },                                              -- CmpItemKindFolderDefault xxx links to CmpItemKind
    CmpItemKindFieldDefault { CmpItemKind },                                               -- CmpItemKindFieldDefault xxx links to CmpItemKind
    CmpItemKindEventDefault { CmpItemKind },                                               -- CmpItemKindEventDefault xxx links to CmpItemKind
    CmpItemKindEnumMemberDefault { CmpItemKind },                                          -- CmpItemKindEnumMemberDefault xxx links to CmpItemKind
    CmpItemKindEnumDefault { CmpItemKind },                                                -- CmpItemKindEnumDefault xxx links to CmpItemKind
    CmpItemKindOperatorDefault { CmpItemKind },                                            -- CmpItemKindOperatorDefault xxx links to CmpItemKind
    CmpItemKindUnitDefault { CmpItemKind },                                                -- CmpItemKindUnitDefault xxx links to CmpItemKind
    CmpItemKindValueDefault { CmpItemKind },                                               -- CmpItemKindValueDefault xxx links to CmpItemKind
    CmpItemAbbrMatchFuzzy { gui = "bold", fg = "#ffeeee", },                               -- CmpItemAbbrMatchFuzzy xxx cterm=bold gui=bold guifg=#ffeeee
    CmpItemAbbrMatch { gui = "bold", fg = "#ffeeee", },                                    -- CmpItemAbbrMatch xxx cterm=bold gui=bold guifg=#ffeeee
    CmpItemAbbrDeprecated { gui = "strikethrough", fg = "#df8489", },                      -- CmpItemAbbrDeprecated xxx cterm=strikethrough gui=strikethrough guifg=#df8489
    CmpItemAbbr { fg = "#8b7079", },                                                       -- CmpItemAbbr    xxx guifg=#8b7079
    DevIconPackageLockJson { fg = "#7a0d21", },                                            -- DevIconPackageLockJson xxx ctermfg=52 guifg=#7a0d21
    DevIconPackageJson { fg = "#e8274b", },                                                -- DevIconPackageJson xxx ctermfg=197 guifg=#e8274b
    DevIconMd { fg = "#dddddd", },                                                         -- DevIconMd      xxx ctermfg=253 guifg=#dddddd
    DevIconZip { fg = "#f9d71c", },                                                        -- DevIconZip     xxx guifg=#f9d71c
    DevIconXz { fg = "#f9d71c", },                                                         -- DevIconXz      xxx guifg=#f9d71c
    DevIconWebOpenFontFormat2 { fg = "#abb2bf", },                                         -- DevIconWebOpenFontFormat2 xxx guifg=#abb2bf
    DevIconWebOpenFontFormat { fg = "#abb2bf", },                                          -- DevIconWebOpenFontFormat xxx guifg=#abb2bf
    DevIconVue { fg = "#8dc149", },                                                        -- DevIconVue     xxx ctermfg=113 guifg=#8dc149
    DevIconTs { fg = "#519aba", },                                                         -- DevIconTs      xxx guifg=#519aba
    DevIconTrueTypeFont { fg = "#abb2bf", },                                               -- DevIconTrueTypeFont xxx guifg=#abb2bf
    DevIconToml { fg = "#9c4221", },                                                       -- DevIconToml    xxx ctermfg=124 guifg=#9c4221
    StatusInsert { bg = "#9ac374", fg = "#281e22", },                                      -- StatusInsert   xxx guifg=#281e22 guibg=#9ac374
    StatusTerminal { StatusInsert },                                                       -- StatusTerminal xxx links to StatusInsert
    HeirlineInsert { StatusInsert },                                                       -- HeirlineInsert xxx links to StatusInsert
    StatusNormal { bg = "#9cbdc9", fg = "#281e22", },                                      -- StatusNormal   xxx guifg=#281e22 guibg=#9cbdc9
    HeirlineNormal { StatusNormal },                                                       -- HeirlineNormal xxx links to StatusNormal
    DevIconOut { fg = "#abb2bf", },                                                        -- DevIconOut     xxx guifg=#abb2bf
    DevIconMp3 { fg = "#d39ede", },                                                        -- DevIconMp3     xxx guifg=#d39ede
    GitSignsChange { fg = "#faa27f", },                                                    -- GitSignsChange xxx guifg=#faa27f
    GitSignsAdd { fg = "#9ac374", },                                                       -- GitSignsAdd    xxx guifg=#9ac374
    Bold { gui = "bold", },                                                                -- Bold           xxx cterm=bold gui=bold
    NeoTreeWinSeparator { gui = "bold", bg = "#1e1517", fg = "#281e22", },                 -- NeoTreeWinSeparator xxx cterm=bold gui=bold guifg=#281e22 guibg=#1e1517
    NeoTreeVertSplit { bg = "#1e1517", fg = "#281e22", },                                  -- NeoTreeVertSplit xxx guifg=#281e22 guibg=#1e1517
    NeoTreeTabSeparatorInactive { bg = "#281e22", fg = "#281e22", },                       -- NeoTreeTabSeparatorInactive xxx guifg=#281e22 guibg=#281e22
    NeoTreeTabSeparatorActive { bg = "#1a1113", fg = "#1a1113", },                         -- NeoTreeTabSeparatorActive xxx guifg=#1a1113 guibg=#1a1113
    NeoTreeTabInactive { bg = "#281e22", fg = "#68545b", },                                -- NeoTreeTabInactive xxx guifg=#68545b guibg=#281e22
    NeoTreeTabActive { gui = "bold", bg = "#1a1113", fg = "#dadada", },                    -- NeoTreeTabActive xxx cterm=bold gui=bold guifg=#dadada guibg=#1a1113
    NeoTreeSymbolicLinkTarget { fg = "#88c3ab", },                                         -- NeoTreeSymbolicLinkTarget xxx guifg=#88c3ab
    NeoTreeNormalNC { bg = "#1a1113", fg = "#8b7079", },                                   -- NeoTreeNormalNC xxx guifg=#8b7079 guibg=#1a1113
    NeoTreeNormal { bg = "#1a1113", fg = "#8b7079", },                                     -- NeoTreeNormal  xxx guifg=#8b7079 guibg=#1a1113
    NeoTreeTitleBar { gui = "bold", bg = "#1a1113", fg = "#9cbdc9", },                     -- NeoTreeTitleBar xxx cterm=bold gui=bold guifg=#9cbdc9 guibg=#1a1113
    NeoTreeGitUnstaged { fg = "#ffd770", },                                                -- NeoTreeGitUnstaged xxx guifg=#ffd770
    NeoTreeGitUntracked { fg = "#ffd770", },                                               -- NeoTreeGitUntracked xxx guifg=#ffd770
    NeoTreeGitStaged { fg = "#88c3ab", },                                                  -- NeoTreeGitStaged xxx guifg=#88c3ab
    NeoTreeGitRenamed { fg = "#faa27f", },                                                 -- NeoTreeGitRenamed xxx guifg=#faa27f
    NeoTreeGitModified { fg = "#faa27f", },                                                -- NeoTreeGitModified xxx guifg=#faa27f
    NeoTreeGitIgnored { fg = "#8b7079", },                                                 -- NeoTreeGitIgnored xxx guifg=#8b7079
    NeoTreeGitDeleted { fg = "#68545b", },                                                 -- NeoTreeGitDeleted xxx guifg=#68545b
    NeoTreeGitConflict { fg = "#e77777", },                                                -- NeoTreeGitConflict xxx guifg=#e77777
    NeoTreeGitAdded { fg = "#9ac374", },                                                   -- NeoTreeGitAdded xxx guifg=#9ac374
    NeoTreeIndentMarker { fg = "#393337", },                                               -- NeoTreeIndentMarker xxx guifg=#393337
    NeoTreeFloatTitle { gui = "bold", bg = "#1a1113", fg = "#9cbdc9", },                   -- NeoTreeFloatTitle xxx cterm=bold gui=bold guifg=#9cbdc9 guibg=#1a1113
    NeoTreeFloatBorder { fg = "#1a1113", },                                                -- NeoTreeFloatBorder xxx guifg=#1a1113
    NeoTreeFileNameOpened { fg = "#9ac374", },                                             -- NeoTreeFileNameOpened xxx guifg=#9ac374
    NeoTreeFileIcon { fg = "#8b7079", },                                                   -- NeoTreeFileIcon xxx guifg=#8b7079
    NeoTreeFileName { fg = "#8b7079", },                                                   -- NeoTreeFileName xxx guifg=#8b7079
    NeoTreeRootName { gui = "bold", fg = "#dadada", },                                     -- NeoTreeRootName xxx cterm=bold gui=bold guifg=#dadada
    NeoTreeDirectoryIcon { fg = "#9cbdc9", },                                              -- NeoTreeDirectoryIcon xxx guifg=#9cbdc9
    LazyUrl { gui = "underline", fg = "#9cbdc9", },                                        -- LazyUrl        xxx cterm=underline gui=underline guifg=#9cbdc9
    LazyDir { fg = "#faa27f", },                                                           -- LazyDir        xxx guifg=#faa27f
    LazyTaskError { fg = "#e77777", },                                                     -- LazyTaskError  xxx guifg=#e77777
    LazyTaskOutput { fg = "#8b7079", },                                                    -- LazyTaskOutput xxx guifg=#8b7079
    LazyReasonImport { fg = "#8b7079", },                                                  -- LazyReasonImport xxx guifg=#8b7079
    LazyReasonCmd { fg = "#faa27f", },                                                     -- LazyReasonCmd  xxx guifg=#faa27f
    LazyReasonFt { fg = "#88c3ab", },                                                      -- LazyReasonFt   xxx guifg=#88c3ab
    LazyReasonSource { fg = "#9ac374", },                                                  -- LazyReasonSource xxx guifg=#9ac374
    LazyReasonStart { fg = "#e77777", },                                                   -- LazyReasonStart xxx guifg=#e77777
    LazyReasonKeys { fg = "#efbde3", },                                                    -- LazyReasonKeys xxx guifg=#efbde3
    LazyReasonEvent { fg = "#ffd770", },                                                   -- LazyReasonEvent xxx guifg=#ffd770
    LazyReasonPlugin { fg = "#9cbdc9", },                                                  -- LazyReasonPlugin xxx guifg=#9cbdc9
    LazyReasonRuntime { fg = "#ffd770", },                                                 -- LazyReasonRuntime xxx guifg=#ffd770
    LazySpecial { fg = "#9cbdc9", },                                                       -- LazySpecial    xxx guifg=#9cbdc9
    LazyProgressDone { fg = "#9ac374", },                                                  -- LazyProgressDone xxx guifg=#9ac374
    LazyLocal { fg = "#ffd770", },                                                         -- LazyLocal      xxx guifg=#ffd770
    LazyValue { fg = "#9ac374", },                                                         -- LazyValue      xxx guifg=#9ac374
    DashboardFooter { fg = "#26c485", },                                                   -- DashboardFooter xxx guifg=#26c485
    DashboardHeader { fg = "#26c485", },                                                   -- DashboardHeader xxx guifg=#26c485
    GitSignsDelete { fg = "#e77777", },                                                    -- GitSignsDelete xxx guifg=#e77777
    StatusInactive { bg = "#68545b", fg = "#281e22", },                                    -- StatusInactive xxx guifg=#281e22 guibg=#68545b
    HeirlineInactive { StatusInactive },                                                   -- HeirlineInactive xxx links to StatusInactive
    IndentBlanklineSpaceCharBlankline { fg = "#393337", },                                 -- IndentBlanklineSpaceCharBlankline xxx guifg=#393337
    DevIconRs { fg = "#dea584", },                                                         -- DevIconRs      xxx ctermfg=216 guifg=#dea584
    DevIconRpm { fg = "#fca2aa", },                                                        -- DevIconRpm     xxx guifg=#fca2aa
    DevIconRobots { fg = "#abb2bf", },                                                     -- DevIconRobots  xxx guifg=#abb2bf
    DevIconRb { fg = "#701516", },                                                         -- DevIconRb      xxx ctermfg=52 guifg=#701516
    DevIconPy { fg = "#ffbc03", },                                                         -- DevIconPy      xxx ctermfg=214 guifg=#ffbc03
    DevIconPng { fg = "#a074c4", },                                                        -- DevIconPng     xxx ctermfg=140 guifg=#a074c4
    DevIconMp4 { fg = "#9ea3de", },                                                        -- DevIconMp4     xxx guifg=#9ea3de
    sym "@string.special.symbol.ruby" { fg = "#ef9474", },                                 -- @string.special.symbol.ruby xxx guifg=#ef9474
    sym "@symbol.ruby" { sym "@string.special.symbol.ruby" },                              -- @symbol.ruby   xxx links to @string.special.symbol.ruby
    sym "@punctuation.delimiter" { fg = "#dadada", },                                      -- @punctuation.delimiter xxx guifg=#dadada
    sym "@punctuation.delimiter.markdown" { sym "@punctuation.delimiter" },                -- @punctuation.delimiter.markdown xxx links to @punctuation.delimiter
    sym "@none.markdown" { fg = "#dadada", },                                              -- @none.markdown xxx guifg=#dadada
    sym "@literal.markdown" { fg = "#6d9f71", },                                           -- @literal.markdown xxx guifg=#6d9f71
    sym "@typeParameter" { fg = "#ffd770", },                                              -- @typeParameter xxx guifg=#ffd770
    sym "@struct" { fg = "#4fa9c6", },                                                     -- @struct        xxx guifg=#4fa9c6
    sym "@regexp" { fg = "#26c485", },                                                     -- @regexp        xxx guifg=#26c485
    sym "@class" { fg = "#4fa9c6", },                                                      -- @class         xxx guifg=#4fa9c6
    sym "@annotation" { fg = "#ffd770", },                                                 -- @annotation    xxx guifg=#ffd770
    DapUIRestart { fg = "#ffd770", },                                                      -- DapUIRestart   xxx guifg=#ffd770
    DapUIStop { fg = "#e77777", },                                                         -- DapUIStop      xxx guifg=#e77777
    DapUIPlayPause { fg = "#9ac374", },                                                    -- DapUIPlayPause xxx guifg=#9ac374
    DapUIBreakpointsInfo { fg = "#efbde3", },                                              -- DapUIBreakpointsInfo xxx guifg=#efbde3
    DapUIWatchesError { fg = "#e77777", },                                                 -- DapUIWatchesError xxx guifg=#e77777
    DapUIWatchesValue { fg = "#faa27f", },                                                 -- DapUIWatchesValue xxx guifg=#faa27f
    DapUIWatchesEmpty { fg = "#e77777", },                                                 -- DapUIWatchesEmpty xxx guifg=#e77777
    sym "@markup.list.checked" { bg = "#9ac374", fg = "#1e1517", },                        -- @markup.list.checked xxx guifg=#1e1517 guibg=#9ac374
    sym "@text.checked" { sym "@markup.list.checked" },                                    -- @text.checked  xxx links to @markup.list.checked
    sym "@markup.list.unchecked" { bg = "#efbde3", fg = "#1e1517", },                      -- @markup.list.unchecked xxx guifg=#1e1517 guibg=#efbde3
    sym "@text.unchecked" { sym "@markup.list.unchecked" },                                -- @text.unchecked xxx links to @markup.list.unchecked
    DevIconLock { fg = "#bbbbbb", },                                                       -- DevIconLock    xxx ctermfg=250 guifg=#bbbbbb
    DevIconKotlin { fg = "#7f52ff", },                                                     -- DevIconKotlin  xxx ctermfg=99 guifg=#7f52ff
    DevIconJsx { fg = "#20c2e3", },                                                        -- DevIconJsx     xxx ctermfg=45 guifg=#20c2e3
    DevIconJs { fg = "#cbcb41", },                                                         -- DevIconJs      xxx ctermfg=185 guifg=#cbcb41
    DevIconJpg { fg = "#a074c4", },                                                        -- DevIconJpg     xxx ctermfg=140 guifg=#a074c4
    sym "@interface" { fg = "#ef9474", },                                                  -- @interface     xxx guifg=#ef9474
    sym "@lsp.type.interface" { sym "@interface" },                                        -- @lsp.type.interface xxx links to @interface
    DapUIBreakpointsCurrentLine { gui = "bold", fg = "#ffd770", },                         -- DapUIBreakpointsCurrentLine xxx cterm=bold gui=bold guifg=#ffd770
    AerialNormal { fg = "#8b7079", },                                                      -- AerialNormal   xxx guifg=#8b7079
    AerialArray { AerialNormal },                                                          -- AerialArray    xxx links to AerialNormal
    AerialBoolean { AerialNormal },                                                        -- AerialBoolean  xxx links to AerialNormal
    AerialClass { AerialNormal },                                                          -- AerialClass    xxx links to AerialNormal
    AerialConstant { AerialNormal },                                                       -- AerialConstant xxx links to AerialNormal
    AerialConstructor { AerialNormal },                                                    -- AerialConstructor xxx links to AerialNormal
    AerialEnum { AerialNormal },                                                           -- AerialEnum     xxx links to AerialNormal
    AerialEnumMember { AerialNormal },                                                     -- AerialEnumMember xxx links to AerialNormal
    AerialEvent { AerialNormal },                                                          -- AerialEvent    xxx links to AerialNormal
    AerialField { AerialNormal },                                                          -- AerialField    xxx links to AerialNormal
    AerialFile { AerialNormal },                                                           -- AerialFile     xxx links to AerialNormal
    AerialFunction { AerialNormal },                                                       -- AerialFunction xxx links to AerialNormal
    AerialInterface { AerialNormal },                                                      -- AerialInterface xxx links to AerialNormal
    AerialKey { AerialNormal },                                                            -- AerialKey      xxx links to AerialNormal
    AerialMethod { AerialNormal },                                                         -- AerialMethod   xxx links to AerialNormal
    AerialModule { AerialNormal },                                                         -- AerialModule   xxx links to AerialNormal
    AerialNamespace { AerialNormal },                                                      -- AerialNamespace xxx links to AerialNormal
    AerialNull { AerialNormal },                                                           -- AerialNull     xxx links to AerialNormal
    AerialNumber { AerialNormal },                                                         -- AerialNumber   xxx links to AerialNormal
    AerialObject { AerialNormal },                                                         -- AerialObject   xxx links to AerialNormal
    AerialOperator { AerialNormal },                                                       -- AerialOperator xxx links to AerialNormal
    AerialPackage { AerialNormal },                                                        -- AerialPackage  xxx links to AerialNormal
    AerialProperty { AerialNormal },                                                       -- AerialProperty xxx links to AerialNormal
    AerialString { AerialNormal },                                                         -- AerialString   xxx links to AerialNormal
    AerialStruct { AerialNormal },                                                         -- AerialStruct   xxx links to AerialNormal
    AerialTypeParameter { AerialNormal },                                                  -- AerialTypeParameter xxx links to AerialNormal
    AerialVariable { AerialNormal },                                                       -- AerialVariable xxx links to AerialNormal
    sym "@markup.math" { fg = "#4fa9c6", },                                                -- @markup.math   xxx guifg=#4fa9c6
    sym "@text.math" { sym "@markup.math" },                                               -- @text.math     xxx links to @markup.math
    DashboardCenter { fg = "#4fa9c6", },                                                   -- DashboardCenter xxx guifg=#4fa9c6
    DashboardShortcut { fg = "#ffd770", },                                                 -- DashboardShortcut xxx guifg=#ffd770
    DiffIndexLine { fg = "#26c485", },                                                     -- DiffIndexLine  xxx guifg=#26c485
    DiffLine { fg = "#dadada", },                                                          -- DiffLine       xxx guifg=#dadada
    DiffFile { fg = "#4fa9c6", },                                                          -- DiffFile       xxx guifg=#4fa9c6
    DiffNewFile { fg = "#6d9f71", },                                                       -- DiffNewFile    xxx guifg=#6d9f71
    DiffOldFile { fg = "#ef9474", },                                                       -- DiffOldFile    xxx guifg=#ef9474
    DiffChanged { fg = "#4fa9c6", },                                                       -- DiffChanged    xxx guifg=#4fa9c6
    DiffRemoved { fg = "#df8489", },                                                       -- DiffRemoved    xxx guifg=#df8489
    DiffAdded { fg = "#6d9f71", },                                                         -- DiffAdded      xxx guifg=#6d9f71
    sym "@diff.plus" { DiffAdded },                                                        -- @diff.plus     xxx links to DiffAdded
    Italic { gui = "italic", },                                                            -- Italic         xxx cterm=italic gui=italic
    DapUIValue { fg = "#df8489", },                                                        -- DapUIValue     xxx guifg=#df8489
    DapUIModifiedValue { gui = "bold", fg = "#ffd770", },                                  -- DapUIModifiedValue xxx cterm=bold gui=bold guifg=#ffd770
    DapUIDecoration { fg = "#393337", },                                                   -- DapUIDecoration xxx guifg=#393337
    IblScope { fg = "#8b7079", },                                                          -- IblScope       xxx guifg=#8b7079
    DevIconLua { fg = "#56889f", },                                                        -- DevIconLua     xxx ctermfg=74 guifg=#56889f
    DevIconJpeg { fg = "#a074c4", },                                                       -- DevIconJpeg    xxx ctermfg=140 guifg=#a074c4
    DevIconHtml { fg = "#e44d26", },                                                       -- DevIconHtml    xxx ctermfg=196 guifg=#e44d26
    DevIconDockerfile { fg = "#458ee6", },                                                 -- DevIconDockerfile xxx ctermfg=68 guifg=#458ee6
    DevIconDeb { fg = "#a1b7ee", },                                                        -- DevIconDeb     xxx guifg=#a1b7ee
    DevIconCss { fg = "#42a5f5", },                                                        -- DevIconCss     xxx ctermfg=75 guifg=#42a5f5
    DevIconC { fg = "#599eff", },                                                          -- DevIconC       xxx ctermfg=111 guifg=#599eff
    DapUIStepOut { fg = "#9cbdc9", },                                                      -- DapUIStepOut   xxx guifg=#9cbdc9
    DapUIStepBack { fg = "#9cbdc9", },                                                     -- DapUIStepBack  xxx guifg=#9cbdc9
    DapUIStepInto { fg = "#9cbdc9", },                                                     -- DapUIStepInto  xxx guifg=#9cbdc9
    DapUIStepOver { fg = "#9cbdc9", },                                                     -- DapUIStepOver  xxx guifg=#9cbdc9
    IblIndent { fg = "#393337", },                                                         -- IblIndent      xxx guifg=#393337
    sym "@module.rust" { fg = "#cd87ba", },                                                -- @module.rust   xxx guifg=#cd87ba
    sym "@namespace.rust" { sym "@module.rust" },                                          -- @namespace.rust xxx links to @module.rust
    sym "@lsp.type.function.rust" { fg = "#dadada", },                                     -- @lsp.type.function.rust xxx guifg=#dadada
    sym "@function.macro" { fg = "#ffd770", },                                             -- @function.macro xxx guifg=#ffd770
    sym "@function.macro.rust" { fg = "#df8489", },                                        -- @function.macro.rust xxx guifg=#df8489
    sym "@lsp.type.macro.rust" { sym "@function.macro.rust" },                             -- @lsp.type.macro.rust xxx links to @function.macro.rust
    sym "@lsp.typemod.function.defaultLibrary.rust" { fg = "#dadada", },                   -- @lsp.typemod.function.defaultLibrary.rust xxx guifg=#dadada
    sym "@modifier" { fg = "#ef9474", },                                                   -- @modifier      xxx guifg=#ef9474
    sym "@event" { fg = "#ef9474", },                                                      -- @event         xxx guifg=#ef9474
    sym "@enumMember" { fg = "#ef9474", },                                                 -- @enumMember    xxx guifg=#ef9474
    sym "@enum" { fg = "#26c485", },                                                       -- @enum          xxx guifg=#26c485
    sym "@decorator" { fg = "#ef9474", },                                                  -- @decorator     xxx guifg=#ef9474
    sym "@comment.hint" { bg = "#9cbdc9", fg = "#1e1517", },                               -- @comment.hint  xxx guifg=#1e1517 guibg=#9cbdc9
    sym "@text.note" { sym "@comment.hint" },                                              -- @text.note     xxx links to @comment.hint
    sym "@markup.link.url" { gui = "underline,italic", fg = "#4fa9c6", },                  -- @markup.link.url xxx cterm=underline,italic gui=underline,italic guifg=#4fa9c6
    sym "@string.special.url" { sym "@markup.link.url" },                                  -- @string.special.url xxx links to @markup.link.url
    sym "@text.uri" { sym "@markup.link.url" },                                            -- @text.uri      xxx links to @markup.link.url
    sym "@markup.raw" { fg = "#df8489", },                                                 -- @markup.raw    xxx guifg=#df8489
    sym "@text.literal" { sym "@markup.raw" },                                             -- @text.literal  xxx links to @markup.raw
    sym "@markup.environment" { fg = "#ef9474", },                                         -- @markup.environment xxx guifg=#ef9474
    sym "@text.environment" { sym "@markup.environment" },                                 -- @text.environment xxx links to @markup.environment
    sym "@text.environment.name" { sym "@markup.environment" },                            -- @text.environment.name xxx links to @markup.environment
    sym "@markup.quote" { gui = "italic", fg = "#dadada", },                               -- @markup.quote  xxx cterm=italic gui=italic guifg=#dadada
    sym "@text.quote" { sym "@markup.quote" },                                             -- @text.quote    xxx links to @markup.quote
    sym "@markup.heading.5.markdown" { gui = "bold", fg = "#ffd770", },                    -- @markup.heading.5.markdown xxx cterm=bold gui=bold guifg=#ffd770
    sym "@text.title.5.markdown" { sym "@markup.heading.5.markdown" },                     -- @text.title.5.markdown xxx links to @markup.heading.5.markdown
    sym "@markup.heading.4.markdown" { gui = "bold", fg = "#6d9f71", },                    -- @markup.heading.4.markdown xxx cterm=bold gui=bold guifg=#6d9f71
    sym "@text.title.4.markdown" { sym "@markup.heading.4.markdown" },                     -- @text.title.4.markdown xxx links to @markup.heading.4.markdown
    sym "@markup.heading.3.markdown" { gui = "bold", fg = "#26c485", },                    -- @markup.heading.3.markdown xxx cterm=bold gui=bold guifg=#26c485
    sym "@text.title.3.markdown" { sym "@markup.heading.3.markdown" },                     -- @text.title.3.markdown xxx links to @markup.heading.3.markdown
    sym "@markup.heading.2.markdown" { gui = "bold", fg = "#4fa9c6", },                    -- @markup.heading.2.markdown xxx cterm=bold gui=bold guifg=#4fa9c6
    sym "@text.title.2.markdown" { sym "@markup.heading.2.markdown" },                     -- @text.title.2.markdown xxx links to @markup.heading.2.markdown
    sym "@markup.heading.1.markdown" { gui = "bold", fg = "#cd87ba", },                    -- @markup.heading.1.markdown xxx cterm=bold gui=bold guifg=#cd87ba
    sym "@text.title.1.markdown" { sym "@markup.heading.1.markdown" },                     -- @text.title.1.markdown xxx links to @markup.heading.1.markdown
    sym "@field.yaml" { fg = "#df8489", },                                                 -- @field.yaml    xxx guifg=#df8489
    sym "@tag.attribute" { fg = "#26c485", },                                              -- @tag.attribute xxx guifg=#26c485
    sym "@tag.attribute.tsx" { gui = "italic", fg = "#cd87ba", },                          -- @tag.attribute.tsx xxx cterm=italic gui=italic guifg=#cd87ba
    sym "@constructor.tsx" { fg = "#cd87ba", },                                            -- @constructor.tsx xxx guifg=#cd87ba
    sym "@constructor.typescript" { fg = "#cd87ba", },                                     -- @constructor.typescript xxx guifg=#cd87ba
    sym "@property.toml" { fg = "#df8489", },                                              -- @property.toml xxx guifg=#df8489
    sym "@type.rust" { fg = "#26c485", },                                                  -- @type.rust     xxx guifg=#26c485
    sym "@punctuation.special.rust" { fg = "#cd87ba", },                                   -- @punctuation.special.rust xxx guifg=#cd87ba
    sym "@constant.rust" { fg = "#26c485", },                                              -- @constant.rust xxx guifg=#26c485
    sym "@constructor.lua" { fg = "#ef9474", },                                            -- @constructor.lua xxx guifg=#ef9474
    sym "@label.json" { fg = "#4fa9c6", },                                                 -- @label.json    xxx guifg=#4fa9c6
    sym "@type.css" { fg = "#ef9474", },                                                   -- @type.css      xxx guifg=#ef9474
    sym "@type.tag.css" { fg = "#cd87ba", },                                               -- @type.tag.css  xxx guifg=#cd87ba
    sym "@string.plain.css" { fg = "#6d9f71", },                                           -- @string.plain.css xxx guifg=#6d9f71
    sym "@property.id.css" { fg = "#4fa9c6", },                                            -- @property.id.css xxx guifg=#4fa9c6
    sym "@property.class.css" { fg = "#ffd770", },                                         -- @property.class.css xxx guifg=#ffd770
    sym "@property.css" { fg = "#ef9474", },                                               -- @property.css  xxx guifg=#ef9474
    sym "@number.css" { fg = "#26c485", },                                                 -- @number.css    xxx guifg=#26c485
    sym "@property.cpp" { fg = "#ef9474", },                                               -- @property.cpp  xxx guifg=#ef9474
    sym "@tag.delimiter" { fg = "#dadada", },                                              -- @tag.delimiter xxx guifg=#dadada
    sym "@markup.raw.block" { fg = "#dadada", },                                           -- @markup.raw.block xxx guifg=#dadada
    sym "@comment.info" { bg = "#88c3ab", fg = "#1e1517", },                               -- @comment.info  xxx guifg=#1e1517 guibg=#88c3ab
    sym "@punctuation.bracket" { fg = "#dadada", },                                        -- @punctuation.bracket xxx guifg=#dadada
    Terminal { bg = "#1e1517", fg = "#8b7079", },                                          -- Terminal       xxx guifg=#8b7079 guibg=#1e1517
    DevIconGradleProperties { fg = "#005f87", },                                           -- DevIconGradleProperties xxx ctermfg=24 guifg=#005f87
    DevIconWindowsMediaAudio { fg = "#00afff", },                                          -- DevIconWindowsMediaAudio xxx ctermfg=39 guifg=#00afff
    DevIconDoc { fg = "#185abd", },                                                        -- DevIconDoc     xxx ctermfg=26 guifg=#185abd
    DevIconWaveformAudioFile { fg = "#00afff", },                                          -- DevIconWaveformAudioFile xxx ctermfg=39 guifg=#00afff
    DevIconVsix { fg = "#854cc7", },                                                       -- DevIconVsix    xxx ctermfg=98 guifg=#854cc7
    DevIconVerilog { fg = "#019833", },                                                    -- DevIconVerilog xxx ctermfg=28 guifg=#019833
    DevIconBzl { fg = "#89e051", },                                                        -- DevIconBzl     xxx ctermfg=113 guifg=#89e051
    DevIconTypoScript { fg = "#ff8700", },                                                 -- DevIconTypoScript xxx ctermfg=208 guifg=#ff8700
    DevIconTxz { fg = "#eca517", },                                                        -- DevIconTxz     xxx ctermfg=214 guifg=#eca517
    DevIconTypoScriptConfig { fg = "#ff8700", },                                           -- DevIconTypoScriptConfig xxx ctermfg=208 guifg=#ff8700
    DevIconLhs { fg = "#a074c4", },                                                        -- DevIconLhs     xxx ctermfg=140 guifg=#a074c4
    DevIconAstro { fg = "#e23f67", },                                                      -- DevIconAstro   xxx ctermfg=197 guifg=#e23f67
    DevIconGDScript { fg = "#6d8086", },                                                   -- DevIconGDScript xxx ctermfg=66 guifg=#6d8086
    DevIconTcl { fg = "#1e5cb3", },                                                        -- DevIconTcl     xxx ctermfg=25 guifg=#1e5cb3
    DevIconSignature { fg = "#e37933", },                                                  -- DevIconSignature xxx ctermfg=166 guifg=#e37933
    DevIconTerminal { fg = "#31b53e", },                                                   -- DevIconTerminal xxx ctermfg=34 guifg=#31b53e
    DevIconTgz { fg = "#eca517", },                                                        -- DevIconTgz     xxx ctermfg=214 guifg=#eca517
    DevIconInfo { fg = "#ffffcd", },                                                       -- DevIconInfo    xxx ctermfg=230 guifg=#ffffcd
    DevIconTerraform { fg = "#5f43e9", },                                                  -- DevIconTerraform xxx ctermfg=93 guifg=#5f43e9
    DevIconTypeScriptReactTest { fg = "#1354bf", },                                        -- DevIconTypeScriptReactTest xxx ctermfg=26 guifg=#1354bf
    DevIconTestTs { fg = "#519aba", },                                                     -- DevIconTestTs  xxx ctermfg=74 guifg=#519aba
    DevIconJavaScriptReactTest { fg = "#20c2e3", },                                        -- DevIconJavaScriptReactTest xxx ctermfg=45 guifg=#20c2e3
    DevIconTestJs { fg = "#cbcb41", },                                                     -- DevIconTestJs  xxx ctermfg=185 guifg=#cbcb41
    DevIconZshenv { fg = "#89e051", },                                                     -- DevIconZshenv  xxx ctermfg=113 guifg=#89e051
    DevIconZshprofile { fg = "#89e051", },                                                 -- DevIconZshprofile xxx ctermfg=113 guifg=#89e051
    DevIconZshrc { fg = "#89e051", },                                                      -- DevIconZshrc   xxx ctermfg=113 guifg=#89e051
    DevIconStep { fg = "#839463", },                                                       -- DevIconStep    xxx ctermfg=101 guifg=#839463
    DevIconSte { fg = "#839463", },                                                        -- DevIconSte     xxx ctermfg=101 guifg=#839463
    DevIcon3DObjectFile { fg = "#888888", },                                               -- DevIcon3DObjectFile xxx ctermfg=102 guifg=#888888
    DevIconTypeScriptReactSpec { fg = "#1354bf", },                                        -- DevIconTypeScriptReactSpec xxx ctermfg=26 guifg=#1354bf
    DevIconSpecTs { fg = "#519aba", },                                                     -- DevIconSpecTs  xxx ctermfg=74 guifg=#519aba
    DevIconJavaScriptReactSpec { fg = "#20c2e3", },                                        -- DevIconJavaScriptReactSpec xxx ctermfg=45 guifg=#20c2e3
    DevIconSpecJs { fg = "#cbcb41", },                                                     -- DevIconSpecJs  xxx ctermfg=185 guifg=#cbcb41
    DevIconR { fg = "#2266ba", },                                                          -- DevIconR       xxx ctermfg=25 guifg=#2266ba
    DevIconSql { fg = "#dad8d8", },                                                        -- DevIconSql     xxx ctermfg=188 guifg=#dad8d8
    DevIconDiff { fg = "#41535b", },                                                       -- DevIconDiff    xxx ctermfg=239 guifg=#41535b
    DevIconSolidWorksPrt { fg = "#839463", },                                              -- DevIconSolidWorksPrt xxx ctermfg=101 guifg=#839463
    DevIconVim { fg = "#019833", },                                                        -- DevIconVim     xxx ctermfg=28 guifg=#019833
    DevIconSolidWorksAsm { fg = "#839463", },                                              -- DevIconSolidWorksAsm xxx ctermfg=101 guifg=#839463
    DevIconSketchUp { fg = "#839463", },                                                   -- DevIconSketchUp xxx ctermfg=101 guifg=#839463
    DevIconSha512 { fg = "#8c86af", },                                                     -- DevIconSha512  xxx ctermfg=103 guifg=#8c86af
    DevIconH { fg = "#a074c4", },                                                          -- DevIconH       xxx ctermfg=140 guifg=#a074c4
    DevIconSha384 { fg = "#8c86af", },                                                     -- DevIconSha384  xxx ctermfg=103 guifg=#8c86af
    DevIconSha256 { fg = "#8c86af", },                                                     -- DevIconSha256  xxx ctermfg=103 guifg=#8c86af
    DevIconJWM { fg = "#0078cd", },                                                        -- DevIconJWM     xxx ctermfg=32 guifg=#0078cd
    DevIconSha1 { fg = "#8c86af", },                                                       -- DevIconSha1    xxx ctermfg=103 guifg=#8c86af
    DevIconSub { fg = "#ffb713", },                                                        -- DevIconSub     xxx ctermfg=214 guifg=#ffb713
    DevIconi3 { fg = "#e8ebee", },                                                         -- DevIconi3      xxx ctermfg=255 guifg=#e8ebee
    DevIconDump { fg = "#dad8d8", },                                                       -- DevIconDump    xxx ctermfg=188 guifg=#dad8d8
    DevIconHyprland { fg = "#00aaae", },                                                   -- DevIconHyprland xxx ctermfg=37 guifg=#00aaae
    DevIconFluxbox { fg = "#555555", },                                                    -- DevIconFluxbox xxx ctermfg=240 guifg=#555555
    DevIcondwm { fg = "#1177aa", },                                                        -- DevIcondwm     xxx ctermfg=31 guifg=#1177aa
    DevIconBSPWM { fg = "#2f2f2f", },                                                      -- DevIconBSPWM   xxx ctermfg=236 guifg=#2f2f2f
    DevIconawesome { fg = "#535d6c", },                                                    -- DevIconawesome xxx ctermfg=59 guifg=#535d6c
    DevIconLog { fg = "#dddddd", },                                                        -- DevIconLog     xxx ctermfg=253 guifg=#dddddd
    DevIconXfce { fg = "#00aadf", },                                                       -- DevIconXfce    xxx ctermfg=74 guifg=#00aadf
    DevIconTypeScript { fg = "#519aba", },                                                 -- DevIconTypeScript xxx ctermfg=74 guifg=#519aba
    DevIconKDEPlasma { fg = "#1b89f4", },                                                  -- DevIconKDEPlasma xxx ctermfg=33 guifg=#1b89f4
    DevIconPl { fg = "#519aba", },                                                         -- DevIconPl      xxx ctermfg=74 guifg=#519aba
    DevIconMATE { fg = "#9bda5c", },                                                       -- DevIconMATE    xxx ctermfg=113 guifg=#9bda5c
    DevIconLXQt { fg = "#0191d2", },                                                       -- DevIconLXQt    xxx ctermfg=32 guifg=#0191d2
    DevIconLXDE { fg = "#a4a4a4", },                                                       -- DevIconLXDE    xxx ctermfg=248 guifg=#a4a4a4
    DevIconFish { fg = "#4d5a5e", },                                                       -- DevIconFish    xxx ctermfg=240 guifg=#4d5a5e
    DevIconGNOME { fg = "#ffffff", },                                                      -- DevIconGNOME   xxx ctermfg=231 guifg=#ffffff
    DevIconCinnamon { fg = "#dc682e", },                                                   -- DevIconCinnamon xxx ctermfg=166 guifg=#dc682e
    DevIconFennel { fg = "#fff3d7", },                                                     -- DevIconFennel  xxx ctermfg=230 guifg=#fff3d7
    DevIconBudgie { fg = "#4e5361", },                                                     -- DevIconBudgie  xxx ctermfg=240 guifg=#4e5361
    DevIconCMake { fg = "#6d8086", },                                                      -- DevIconCMake   xxx ctermfg=66 guifg=#6d8086
    DevIconZorin { fg = "#14a1e8", },                                                      -- DevIconZorin   xxx ctermfg=39 guifg=#14a1e8
    DevIconAsciinema { fg = "#fd971f", },                                                  -- DevIconAsciinema xxx ctermfg=208 guifg=#fd971f
    DevIconErb { fg = "#701516", },                                                        -- DevIconErb     xxx ctermfg=52 guifg=#701516
    DevIconHrl { fg = "#b83998", },                                                        -- DevIconHrl     xxx ctermfg=163 guifg=#b83998
    DevIconMarkdown { fg = "#dddddd", },                                                   -- DevIconMarkdown xxx ctermfg=253 guifg=#dddddd
    DevIconEpp { fg = "#ffa61a", },                                                        -- DevIconEpp     xxx ctermfg=214 guifg=#ffa61a
    DevIconVanillaOS { fg = "#fabd4d", },                                                  -- DevIconVanillaOS xxx ctermfg=214 guifg=#fabd4d
    DevIconElm { fg = "#519aba", },                                                        -- DevIconElm     xxx ctermfg=74 guifg=#519aba
    DevIconExs { fg = "#a074c4", },                                                        -- DevIconExs     xxx ctermfg=140 guifg=#a074c4
    DevIconElf { fg = "#9f0500", },                                                        -- DevIconElf     xxx ctermfg=124 guifg=#9f0500
    DevIconLeex { fg = "#a074c4", },                                                       -- DevIconLeex    xxx ctermfg=140 guifg=#a074c4
    DevIconTrisquelGNULinux { fg = "#0f58b6", },                                           -- DevIconTrisquelGNULinux xxx ctermfg=25 guifg=#0f58b6
    DevIconGroovy { fg = "#4a687c", },                                                     -- DevIconGroovy  xxx ctermfg=24 guifg=#4a687c
    DevIconTails { fg = "#56347c", },                                                      -- DevIconTails   xxx ctermfg=54 guifg=#56347c
    DevIconSolus { fg = "#4b5163", },                                                      -- DevIconSolus   xxx ctermfg=239 guifg=#4b5163
    DevIconSlackware { fg = "#475fa9", },                                                  -- DevIconSlackware xxx ctermfg=61 guifg=#475fa9
    DevIconLess { fg = "#563d7c", },                                                       -- DevIconLess    xxx ctermfg=54 guifg=#563d7c
    DevIconGv { fg = "#30638e", },                                                         -- DevIconGv      xxx ctermfg=24 guifg=#30638e
    DevIconDot { fg = "#30638e", },                                                        -- DevIconDot     xxx ctermfg=24 guifg=#30638e
    DevIconSabayon { fg = "#c6c6c6", },                                                    -- DevIconSabayon xxx ctermfg=251 guifg=#c6c6c6
    DevIconJustfile { fg = "#6d8086", },                                                   -- DevIconJustfile xxx ctermfg=66 guifg=#6d8086
    DevIconIni { fg = "#6d8086", },                                                        -- DevIconIni     xxx ctermfg=66 guifg=#6d8086
    DevIconBlender { fg = "#ea7600", },                                                    -- DevIconBlender xxx ctermfg=208 guifg=#ea7600
    DevIconLuaurc { fg = "#00a2ff", },                                                     -- DevIconLuaurc  xxx ctermfg=75 guifg=#00a2ff
    DevIconRockyLinux { fg = "#0fb37d", },                                                 -- DevIconRockyLinux xxx ctermfg=36 guifg=#0fb37d
    DevIconEslintrc { fg = "#4b32c3", },                                                   -- DevIconEslintrc xxx ctermfg=56 guifg=#4b32c3
    DevIconDart { fg = "#03589c", },                                                       -- DevIconDart    xxx ctermfg=25 guifg=#03589c
    DevIconPrettierConfig { fg = "#4285f4", },                                             -- DevIconPrettierConfig xxx ctermfg=33 guifg=#4285f4
    DevIconRaspberryPiOS { fg = "#be1848", },                                              -- DevIconRaspberryPiOS xxx ctermfg=161 guifg=#be1848
    DevIconQubesOS { fg = "#3774d8", },                                                    -- DevIconQubesOS xxx ctermfg=33 guifg=#3774d8
    DevIconPhp { fg = "#a074c4", },                                                        -- DevIconPhp     xxx ctermfg=140 guifg=#a074c4
    DevIconcuda { fg = "#89e051", },                                                       -- DevIconcuda    xxx ctermfg=113 guifg=#89e051
    DevIconcudah { fg = "#a074c4", },                                                      -- DevIconcudah   xxx ctermfg=140 guifg=#a074c4
    DevIconCsv { fg = "#89e051", },                                                        -- DevIconCsv     xxx ctermfg=113 guifg=#89e051
    DevIconRazorPage { fg = "#512bd4", },                                                  -- DevIconRazorPage xxx ctermfg=56 guifg=#512bd4
    DevIconCs { fg = "#596706", },                                                         -- DevIconCs      xxx ctermfg=58 guifg=#596706
    DevIconCrystal { fg = "#c8c8c8", },                                                    -- DevIconCrystal xxx ctermfg=251 guifg=#c8c8c8
    DevIconConfig { fg = "#6d8086", },                                                     -- DevIconConfig  xxx ctermfg=66 guifg=#6d8086
    DevIconCppm { fg = "#519aba", },                                                       -- DevIconCppm    xxx ctermfg=74 guifg=#519aba
    DevIconCPlusPlusModule { fg = "#f34b7d", },                                            -- DevIconCPlusPlusModule xxx ctermfg=204 guifg=#f34b7d
    DevIconHpp { fg = "#a074c4", },                                                        -- DevIconHpp     xxx ctermfg=140 guifg=#a074c4
    DevIconBash { fg = "#89e051", },                                                       -- DevIconBash    xxx ctermfg=113 guifg=#89e051
    DevIconGitIgnore { fg = "#f54d27", },                                                  -- DevIconGitIgnore xxx ctermfg=196 guifg=#f54d27
    DevIconGitAttributes { fg = "#f54d27", },                                              -- DevIconGitAttributes xxx ctermfg=196 guifg=#f54d27
    DevIconGitModules { fg = "#f54d27", },                                                 -- DevIconGitModules xxx ctermfg=196 guifg=#f54d27
    DevIconGitConfig { fg = "#f54d27", },                                                  -- DevIconGitConfig xxx ctermfg=196 guifg=#f54d27
    DevIconHh { fg = "#a074c4", },                                                         -- DevIconHh      xxx ctermfg=140 guifg=#a074c4
    DevIconCobol { fg = "#005ca5", },                                                      -- DevIconCobol   xxx ctermfg=25 guifg=#005ca5
    DevIconLib { fg = "#4d2c0b", },                                                        -- DevIconLib     xxx ctermfg=52 guifg=#4d2c0b
    DevIconMixLock { fg = "#a074c4", },                                                    -- DevIconMixLock xxx ctermfg=140 guifg=#a074c4
    DevIconClojureJS { fg = "#519aba", },                                                  -- DevIconClojureJS xxx ctermfg=74 guifg=#519aba
    DevIconEditorConfig { fg = "#fff2f2", },                                               -- DevIconEditorConfig xxx ctermfg=255 guifg=#fff2f2
    DevIconClojure { fg = "#8dc149", },                                                    -- DevIconClojure xxx ctermfg=113 guifg=#8dc149
    DevIconScheme { fg = "#eeeeee", },                                                     -- DevIconScheme  xxx ctermfg=255 guifg=#eeeeee
    DevIconsbt { fg = "#cc3e44", },                                                        -- DevIconsbt     xxx ctermfg=167 guifg=#cc3e44
    DevIconPyx { fg = "#5aa7e4", },                                                        -- DevIconPyx     xxx ctermfg=39 guifg=#5aa7e4
    DevIconRake { fg = "#701516", },                                                       -- DevIconRake    xxx ctermfg=52 guifg=#701516
    DevIconPxd { fg = "#5aa7e4", },                                                        -- DevIconPxd     xxx ctermfg=39 guifg=#5aa7e4
    DevIconGemspec { fg = "#701516", },                                                    -- DevIconGemspec xxx ctermfg=52 guifg=#701516
    DevIconBackup { fg = "#6d8086", },                                                     -- DevIconBackup  xxx ctermfg=66 guifg=#6d8086
    DevIconPsScriptfile { fg = "#4273ca", },                                               -- DevIconPsScriptfile xxx ctermfg=68 guifg=#4273ca
    DevIconPsScriptModulefile { fg = "#6975c4", },                                         -- DevIconPsScriptModulefile xxx ctermfg=68 guifg=#6975c4
    DevIconPsManifestfile { fg = "#6975c4", },                                             -- DevIconPsManifestfile xxx ctermfg=68 guifg=#6975c4
    DevIconPm { fg = "#519aba", },                                                         -- DevIconPm      xxx ctermfg=74 guifg=#519aba
    DevIconOpenSCAD { fg = "#f9d72c", },                                                   -- DevIconOpenSCAD xxx ctermfg=220 guifg=#f9d72c
    DevIconArch { fg = "#0f94d2", },                                                       -- DevIconArch    xxx ctermfg=67 guifg=#0f94d2
    DevIconMl { fg = "#e37933", },                                                         -- DevIconMl      xxx ctermfg=166 guifg=#e37933
    DevIconMli { fg = "#e37933", },                                                        -- DevIconMli     xxx ctermfg=166 guifg=#e37933
    DevIconAi { fg = "#cbcb41", },                                                         -- DevIconAi      xxx ctermfg=185 guifg=#cbcb41
    DevIconLinux { fg = "#fdfdfb", },                                                      -- DevIconLinux   xxx ctermfg=231 guifg=#fdfdfb
    DevIconPatch { fg = "#41535b", },                                                      -- DevIconPatch   xxx ctermfg=239 guifg=#41535b
    DevIconPp { fg = "#ffa61a", },                                                         -- DevIconPp      xxx ctermfg=214 guifg=#ffa61a
    DevIconSml { fg = "#e37933", },                                                        -- DevIconSml     xxx ctermfg=166 guifg=#e37933
    DevIconLocalization { fg = "#2596be", },                                               -- DevIconLocalization xxx ctermfg=31 guifg=#2596be
    DevIconPls { fg = "#ed95ae", },                                                        -- DevIconPls     xxx ctermfg=211 guifg=#ed95ae
    DevIconSig { fg = "#e37933", },                                                        -- DevIconSig     xxx ctermfg=166 guifg=#e37933
    DevIconKsh { fg = "#4d5a5e", },                                                        -- DevIconKsh     xxx ctermfg=240 guifg=#4d5a5e
    DevIconArduino { fg = "#56b6c2", },                                                    -- DevIconArduino xxx ctermfg=73 guifg=#56b6c2
    DevIconReScriptInterface { fg = "#f55385", },                                          -- DevIconReScriptInterface xxx ctermfg=204 guifg=#f55385
    DevIconBibTeX { fg = "#cbcb41", },                                                     -- DevIconBibTeX  xxx ctermfg=185 guifg=#cbcb41
    DevIconMojo { fg = "#ff4c1f", },                                                       -- DevIconMojo    xxx ctermfg=196 guifg=#ff4c1f
    DevIconPrisma { fg = "#5a67d8", },                                                     -- DevIconPrisma  xxx ctermfg=62 guifg=#5a67d8
    DevIconPdf { fg = "#b30b00", },                                                        -- DevIconPdf     xxx ctermfg=124 guifg=#b30b00
    DevIconQt { fg = "#40cd52", },                                                         -- DevIconQt      xxx ctermfg=77 guifg=#40cd52
    DevIconMotoko { fg = "#9772fb", },                                                     -- DevIconMotoko  xxx ctermfg=135 guifg=#9772fb
    DevIconPyi { fg = "#ffbc03", },                                                        -- DevIconPyi     xxx ctermfg=214 guifg=#ffbc03
    DevIconAppleScript { fg = "#6d8085", },                                                -- DevIconAppleScript xxx ctermfg=66 guifg=#6d8085
    DevIconGodotTextResource { fg = "#6d8086", },                                          -- DevIconGodotTextResource xxx ctermfg=66 guifg=#6d8086
    DevIconSvg { fg = "#ffb13b", },                                                        -- DevIconSvg     xxx ctermfg=214 guifg=#ffb13b
    DevIconElisp { fg = "#8172be", },                                                      -- DevIconElisp   xxx ctermfg=97 guifg=#8172be
    DevIconAwk { fg = "#4d5a5e", },                                                        -- DevIconAwk     xxx ctermfg=240 guifg=#4d5a5e
    DevIconHbs { fg = "#f0772b", },                                                        -- DevIconHbs     xxx ctermfg=202 guifg=#f0772b
    DevIconBicep { fg = "#519aba", },                                                      -- DevIconBicep   xxx ctermfg=74 guifg=#519aba
    DevIconHs { fg = "#a074c4", },                                                         -- DevIconHs      xxx ctermfg=140 guifg=#a074c4
    DevIconBlueprint { fg = "#5796e2", },                                                  -- DevIconBlueprint xxx ctermfg=68 guifg=#5796e2
    DevIconBazel { fg = "#89e051", },                                                      -- DevIconBazel   xxx ctermfg=113 guifg=#89e051
    DevIconSharedObject { fg = "#dcddd6", },                                               -- DevIconSharedObject xxx ctermfg=253 guifg=#dcddd6
    DevIconDll { fg = "#4d2c0b", },                                                        -- DevIconDll     xxx ctermfg=52 guifg=#4d2c0b
    DevIconLuau { fg = "#00a2ff", },                                                       -- DevIconLuau    xxx ctermfg=75 guifg=#00a2ff
    DevIconLrc { fg = "#ffb713", },                                                        -- DevIconLrc     xxx ctermfg=214 guifg=#ffb713
    DevIconOpusAudioFile { fg = "#0075aa", },                                              -- DevIconOpusAudioFile xxx ctermfg=24 guifg=#0075aa
    DevIconMint { fg = "#87c095", },                                                       -- DevIconMint    xxx ctermfg=108 guifg=#87c095
    DevIconM3u8 { fg = "#ed95ae", },                                                       -- DevIconM3u8    xxx ctermfg=211 guifg=#ed95ae
    DevIconIco { fg = "#cbcb41", },                                                        -- DevIconIco     xxx ctermfg=185 guifg=#cbcb41
    DevIconM3u { fg = "#ed95ae", },                                                        -- DevIconM3u     xxx ctermfg=211 guifg=#ed95ae
    DevIconAvif { fg = "#a074c4", },                                                       -- DevIconAvif    xxx ctermfg=140 guifg=#a074c4
    DevIconBrewfile { fg = "#701516", },                                                   -- DevIconBrewfile xxx ctermfg=52 guifg=#701516
    DevIconFsx { fg = "#519aba", },                                                        -- DevIconFsx     xxx ctermfg=74 guifg=#519aba
    DevIconPyc { fg = "#ffe291", },                                                        -- DevIconPyc     xxx ctermfg=222 guifg=#ffe291
    DevIconFsi { fg = "#519aba", },                                                        -- DevIconFsi     xxx ctermfg=74 guifg=#519aba
    DevIconPyo { fg = "#ffe291", },                                                        -- DevIconPyo     xxx ctermfg=222 guifg=#ffe291
    DevIconLicense { fg = "#cbcb41", },                                                    -- DevIconLicense xxx ctermfg=185 guifg=#cbcb41
    DevIconMakefile { fg = "#6d8086", },                                                   -- DevIconMakefile xxx ctermfg=66 guifg=#6d8086
    DevIconHaxe { fg = "#ea8220", },                                                       -- DevIconHaxe    xxx ctermfg=208 guifg=#ea8220
    DevIconGodotProject { fg = "#6d8086", },                                               -- DevIconGodotProject xxx ctermfg=66 guifg=#6d8086
    DevIconBinaryGLTF { fg = "#ffb13b", },                                                 -- DevIconBinaryGLTF xxx ctermfg=214 guifg=#ffb13b
    DevIconGif { fg = "#a074c4", },                                                        -- DevIconGif     xxx ctermfg=140 guifg=#a074c4
    DevIconFsscript { fg = "#519aba", },                                                   -- DevIconFsscript xxx ctermfg=74 guifg=#519aba
    DevIconEjs { fg = "#cbcb41", },                                                        -- DevIconEjs     xxx ctermfg=185 guifg=#cbcb41
    DevIconDropbox { fg = "#0061fe", },                                                    -- DevIconDropbox xxx ctermfg=27 guifg=#0061fe
    DevIconDrools { fg = "#ffafaf", },                                                     -- DevIconDrools  xxx ctermfg=217 guifg=#ffafaf
    DevIconDocx { fg = "#185abd", },                                                       -- DevIconDocx    xxx ctermfg=26 guifg=#185abd
    DevIconCson { fg = "#cbcb41", },                                                       -- DevIconCson    xxx ctermfg=185 guifg=#cbcb41
    DevIconCp { fg = "#519aba", },                                                         -- DevIconCp      xxx ctermfg=74 guifg=#519aba
    DevIconCoffee { fg = "#cbcb41", },                                                     -- DevIconCoffee  xxx ctermfg=185 guifg=#cbcb41
    DevIconBmp { fg = "#a074c4", },                                                        -- DevIconBmp     xxx ctermfg=140 guifg=#a074c4
    DevIconVagrantfile { fg = "#1563ff", },                                                -- DevIconVagrantfile xxx ctermfg=27 guifg=#1563ff
    DevIconGemfile { fg = "#701516", },                                                    -- DevIconGemfile xxx ctermfg=52 guifg=#701516
    DevIconRlib { fg = "#dea584", },                                                       -- DevIconRlib    xxx ctermfg=216 guifg=#dea584
    DevIconNodeModules { fg = "#e8274b", },                                                -- DevIconNodeModules xxx ctermfg=197 guifg=#e8274b
    DevIconSlim { fg = "#e34c26", },                                                       -- DevIconSlim    xxx ctermfg=196 guifg=#e34c26
    DevIconConf { fg = "#6d8086", },                                                       -- DevIconConf    xxx ctermfg=66 guifg=#6d8086
    DevIconWasm { fg = "#5c4cdb", },                                                       -- DevIconWasm    xxx ctermfg=62 guifg=#5c4cdb
    DevIconNix { fg = "#7ebae4", },                                                        -- DevIconNix     xxx ctermfg=110 guifg=#7ebae4
    DevIconWebp { fg = "#a074c4", },                                                       -- DevIconWebp    xxx ctermfg=140 guifg=#a074c4
    DevIconWebm { fg = "#fd971f", },                                                       -- DevIconWebm    xxx ctermfg=208 guifg=#fd971f
    DevIconNim { fg = "#f3d400", },                                                        -- DevIconNim     xxx ctermfg=220 guifg=#f3d400
    DevIconFreeCADConfig { fg = "#cb0d0d", },                                              -- DevIconFreeCADConfig xxx ctermfg=160 guifg=#cb0d0d
    DevIconPsb { fg = "#519aba", },                                                        -- DevIconPsb     xxx ctermfg=74 guifg=#519aba
    DevIconKiCadFootprintTable { fg = "#ffffff", },                                        -- DevIconKiCadFootprintTable xxx ctermfg=231 guifg=#ffffff
    DevIconPackedResource { fg = "#6d8086", },                                             -- DevIconPackedResource xxx ctermfg=66 guifg=#6d8086
    DevIconKiCadCache { fg = "#ffffff", },                                                 -- DevIconKiCadCache xxx ctermfg=231 guifg=#ffffff
    DevIconPpt { fg = "#cb4a32", },                                                        -- DevIconPpt     xxx ctermfg=160 guifg=#cb4a32
    DevIconFsharp { fg = "#519aba", },                                                     -- DevIconFsharp  xxx ctermfg=74 guifg=#519aba
    DevIconTypoScriptSetup { fg = "#ff8700", },                                            -- DevIconTypoScriptSetup xxx ctermfg=208 guifg=#ff8700
    DevIconXlsx { fg = "#207245", },                                                       -- DevIconXlsx    xxx ctermfg=29 guifg=#207245
    DevIconCheckhealth { fg = "#75b4fb", },                                                -- DevIconCheckhealth xxx ctermfg=75 guifg=#75b4fb
    DevIconCMakeLists { fg = "#6d8086", },                                                 -- DevIconCMakeLists xxx ctermfg=66 guifg=#6d8086
    DevIconNotebook { fg = "#56889f", },                                                   -- DevIconNotebook xxx ctermfg=74 guifg=#56889f
    DevIconWebmanifest { fg = "#f1e05a", },                                                -- DevIconWebmanifest xxx ctermfg=185 guifg=#f1e05a
    DevIconJava { fg = "#cc3e44", },                                                       -- DevIconJava    xxx ctermfg=167 guifg=#cc3e44
    DevIconVimrc { fg = "#019833", },                                                      -- DevIconVimrc   xxx ctermfg=28 guifg=#019833
    DevIconnode { fg = "#5fa04e", },                                                       -- DevIconnode    xxx ctermfg=71 guifg=#5fa04e
    DevIconGvimrc { fg = "#019833", },                                                     -- DevIconGvimrc  xxx ctermfg=28 guifg=#019833
    DevIconNPMIgnore { fg = "#e8274b", },                                                  -- DevIconNPMIgnore xxx ctermfg=197 guifg=#e8274b
    DevIconMailmap { fg = "#41535b", },                                                    -- DevIconMailmap xxx ctermfg=239 guifg=#41535b
    DevIconXsession { fg = "#e54d18", },                                                   -- DevIconXsession xxx ctermfg=196 guifg=#e54d18
    DevIconGTK { fg = "#ffffff", },                                                        -- DevIconGTK     xxx ctermfg=231 guifg=#ffffff
    DevIconMjs { fg = "#f1e05a", },                                                        -- DevIconMjs     xxx ctermfg=185 guifg=#f1e05a
    DevIconXauthority { fg = "#e54d18", },                                                 -- DevIconXauthority xxx ctermfg=196 guifg=#e54d18
    DevIconxmonad { fg = "#fd4d5d", },                                                     -- DevIconxmonad  xxx ctermfg=203 guifg=#fd4d5d
    DevIconSRCINFO { fg = "#0f94d2", },                                                    -- DevIconSRCINFO xxx ctermfg=67 guifg=#0f94d2
    DevIconSettingsJson { fg = "#854cc7", },                                               -- DevIconSettingsJson xxx ctermfg=98 guifg=#854cc7
    DevIconEslintIgnore { fg = "#4b32c3", },                                               -- DevIconEslintIgnore xxx ctermfg=56 guifg=#4b32c3
    DevIconDsStore { fg = "#41535b", },                                                    -- DevIconDsStore xxx ctermfg=239 guifg=#41535b
    DevIconBashProfile { fg = "#89e051", },                                                -- DevIconBashProfile xxx ctermfg=113 guifg=#89e051
    DevIconHurl { fg = "#ff0288", },                                                       -- DevIconHurl    xxx ctermfg=198 guifg=#ff0288
    DevIconProlog { fg = "#e4b854", },                                                     -- DevIconProlog  xxx ctermfg=179 guifg=#e4b854
    DevIconGradleBuildScript { fg = "#005f87", },                                          -- DevIconGradleBuildScript xxx ctermfg=24 guifg=#005f87
    DevIconDb { fg = "#dad8d8", },                                                         -- DevIconDb      xxx ctermfg=188 guifg=#dad8d8
    DevIconHexadecimal { fg = "#2e63ff", },                                                -- DevIconHexadecimal xxx ctermfg=27 guifg=#2e63ff
    DevIconCSharpProject { fg = "#512bd4", },                                              -- DevIconCSharpProject xxx ctermfg=56 guifg=#512bd4
    DevIconHeex { fg = "#a074c4", },                                                       -- DevIconHeex    xxx ctermfg=140 guifg=#a074c4
    DevIconRss { fg = "#fb9d3b", },                                                        -- DevIconRss     xxx ctermfg=215 guifg=#fb9d3b
    DevIconXul { fg = "#e37933", },                                                        -- DevIconXul     xxx ctermfg=166 guifg=#e37933
    DevIconThunderbird { fg = "#137be1", },                                                -- DevIconThunderbird xxx ctermfg=33 guifg=#137be1
    DevIconM4V { fg = "#fd971f", },                                                        -- DevIconM4V     xxx ctermfg=208 guifg=#fd971f
    DevIconMPEG4 { fg = "#00afff", },                                                      -- DevIconMPEG4   xxx ctermfg=39 guifg=#00afff
    DevIconHaml { fg = "#eaeae1", },                                                       -- DevIconHaml    xxx ctermfg=255 guifg=#eaeae1
    DevIconVRML { fg = "#888888", },                                                       -- DevIconVRML    xxx ctermfg=102 guifg=#888888
    DevIconScala { fg = "#cc3e44", },                                                      -- DevIconScala   xxx ctermfg=167 guifg=#cc3e44
    DevIconGraphQL { fg = "#e535ab", },                                                    -- DevIconGraphQL xxx ctermfg=199 guifg=#e535ab
    DevIconSass { fg = "#f55385", },                                                       -- DevIconSass    xxx ctermfg=204 guifg=#f55385
    DevIconMpp { fg = "#519aba", },                                                        -- DevIconMpp     xxx ctermfg=74 guifg=#519aba
    DevIconXSettingsdConf { fg = "#e54d18", },                                             -- DevIconXSettingsdConf xxx ctermfg=196 guifg=#e54d18
    DevIconCsh { fg = "#4d5a5e", },                                                        -- DevIconCsh     xxx ctermfg=240 guifg=#4d5a5e
    DevIconLibrecadFontFile { fg = "#ececec", },                                           -- DevIconLibrecadFontFile xxx ctermfg=255 guifg=#ececec
    DevIconVHDL { fg = "#019833", },                                                       -- DevIconVHDL    xxx ctermfg=28 guifg=#019833
    DevIconTorrent { fg = "#44cda8", },                                                    -- DevIconTorrent xxx ctermfg=43 guifg=#44cda8
    DevIconGradleSettings { fg = "#005f87", },                                             -- DevIconGradleSettings xxx ctermfg=24 guifg=#005f87
    DevIconJson { fg = "#cbcb41", },                                                       -- DevIconJson    xxx ctermfg=185 guifg=#cbcb41
    DevIconXls { fg = "#207245", },                                                        -- DevIconXls     xxx ctermfg=29 guifg=#207245
    DevIconFortran { fg = "#734f96", },                                                    -- DevIconFortran xxx ctermfg=97 guifg=#734f96
    DevIconLogos { fg = "#519aba", },                                                      -- DevIconLogos   xxx ctermfg=74 guifg=#519aba
    DevIconXresources { fg = "#e54d18", },                                                 -- DevIconXresources xxx ctermfg=196 guifg=#e54d18
    DevIconEnv { fg = "#faf743", },                                                        -- DevIconEnv     xxx ctermfg=227 guifg=#faf743
    DevIconGitLogo { fg = "#f14c28", },                                                    -- DevIconGitLogo xxx ctermfg=196 guifg=#f14c28
    DevIconRar { fg = "#eca517", },                                                        -- DevIconRar     xxx ctermfg=214 guifg=#eca517
    DevIconMdx { fg = "#519aba", },                                                        -- DevIconMdx     xxx ctermfg=74 guifg=#519aba
    DevIconXaml { fg = "#512bd4", },                                                       -- DevIconXaml    xxx ctermfg=56 guifg=#512bd4
    DevIconMaterial { fg = "#b83998", },                                                   -- DevIconMaterial xxx ctermfg=163 guifg=#b83998
    DevIconIcal { fg = "#2b2e83", },                                                       -- DevIconIcal    xxx ctermfg=18 guifg=#2b2e83
    DevIconRproj { fg = "#358a5b", },                                                      -- DevIconRproj   xxx ctermfg=29 guifg=#358a5b
    DevIconNushell { fg = "#3aa675", },                                                    -- DevIconNushell xxx ctermfg=36 guifg=#3aa675
    DevIconRmd { fg = "#519aba", },                                                        -- DevIconRmd     xxx ctermfg=74 guifg=#519aba
    DevIconSh { fg = "#4d5a5e", },                                                         -- DevIconSh      xxx ctermfg=240 guifg=#4d5a5e
    DevIconGradleWrapperScript { fg = "#005f87", },                                        -- DevIconGradleWrapperScript xxx ctermfg=24 guifg=#005f87
    DevIconKrita { fg = "#f245fb", },                                                      -- DevIconKrita   xxx ctermfg=201 guifg=#f245fb
    DevIconLinuxKernelObject { fg = "#dcddd6", },                                          -- DevIconLinuxKernelObject xxx ctermfg=253 guifg=#dcddd6
    DevIconXeroLinux { fg = "#888fe2", },                                                  -- DevIconXeroLinux xxx ctermfg=104 guifg=#888fe2
    DevIconPxi { fg = "#5aa7e4", },                                                        -- DevIconPxi     xxx ctermfg=39 guifg=#5aa7e4
    DevIconKiCad { fg = "#ffffff", },                                                      -- DevIconKiCad   xxx ctermfg=231 guifg=#ffffff
    DevIconEex { fg = "#a074c4", },                                                        -- DevIconEex     xxx ctermfg=140 guifg=#a074c4
    DevIconOggVorbis { fg = "#0075aa", },                                                  -- DevIconOggVorbis xxx ctermfg=24 guifg=#0075aa
    DevIconKdenlive { fg = "#83b8f2", },                                                   -- DevIconKdenlive xxx ctermfg=110 guifg=#83b8f2
    DevIconPub { fg = "#e3c58e", },                                                        -- DevIconPub     xxx ctermfg=222 guifg=#e3c58e
    DevIconTempl { fg = "#dbbd30", },                                                      -- DevIconTempl   xxx ctermfg=178 guifg=#dbbd30
    DevIconPyd { fg = "#ffe291", },                                                        -- DevIconPyd     xxx ctermfg=222 guifg=#ffe291
    DevIconKdbx { fg = "#529b34", },                                                       -- DevIconKdbx    xxx ctermfg=71 guifg=#529b34
    DevIconCts { fg = "#519aba", },                                                        -- DevIconCts     xxx ctermfg=74 guifg=#519aba
    DevIconMts { fg = "#519aba", },                                                        -- DevIconMts     xxx ctermfg=74 guifg=#519aba
    DevIconTwig { fg = "#8dc149", },                                                       -- DevIconTwig    xxx ctermfg=113 guifg=#8dc149
    DevIconBat { fg = "#c1f12e", },                                                        -- DevIconBat     xxx ctermfg=191 guifg=#c1f12e
    DevIconKdb { fg = "#529b34", },                                                        -- DevIconKdb     xxx ctermfg=71 guifg=#529b34
    DevIconKbx { fg = "#737672", },                                                        -- DevIconKbx     xxx ctermfg=243 guifg=#737672
    DevIconKotlinScript { fg = "#7f52ff", },                                               -- DevIconKotlinScript xxx ctermfg=99 guifg=#7f52ff
    DevIconJpegXl { fg = "#a074c4", },                                                     -- DevIconJpegXl  xxx ctermfg=140 guifg=#a074c4
    DevIconXpi { fg = "#ff1b01", },                                                        -- DevIconXpi     xxx ctermfg=196 guifg=#ff1b01
    DevIconSln { fg = "#854cc7", },                                                        -- DevIconSln     xxx ctermfg=98 guifg=#854cc7
    DevIconOrgMode { fg = "#77aa99", },                                                    -- DevIconOrgMode xxx ctermfg=73 guifg=#77aa99
    DevIconDesktopEntry { fg = "#563d7c", },                                               -- DevIconDesktopEntry xxx ctermfg=54 guifg=#563d7c
    DevIconBabelrc { fg = "#cbcb41", },                                                    -- DevIconBabelrc xxx ctermfg=185 guifg=#cbcb41
    DevIconBz3 { fg = "#eca517", },                                                        -- DevIconBz3     xxx ctermfg=214 guifg=#eca517
    DevIconJson5 { fg = "#cbcb41", },                                                      -- DevIconJson5   xxx ctermfg=185 guifg=#cbcb41
    DevIconImportConfiguration { fg = "#ececec", },                                        -- DevIconImportConfiguration xxx ctermfg=255 guifg=#ececec
    DevIconFs { fg = "#519aba", },                                                         -- DevIconFs      xxx ctermfg=74 guifg=#519aba
    DevIconBashrc { fg = "#89e051", },                                                     -- DevIconBashrc  xxx ctermfg=113 guifg=#89e051
    DevIconIso { fg = "#d0bec8", },                                                        -- DevIconIso     xxx ctermfg=181 guifg=#d0bec8
    DevIconObjectiveCPlusPlus { fg = "#519aba", },                                         -- DevIconObjectiveCPlusPlus xxx ctermfg=74 guifg=#519aba
    DevIconCache { fg = "#ffffff", },                                                      -- DevIconCache   xxx ctermfg=231 guifg=#ffffff
    DevIconCxx { fg = "#519aba", },                                                        -- DevIconCxx     xxx ctermfg=74 guifg=#519aba
    DevIconDconf { fg = "#ffffff", },                                                      -- DevIconDconf   xxx ctermfg=231 guifg=#ffffff
    DevIconPulseCodeModulation { fg = "#0075aa", },                                        -- DevIconPulseCodeModulation xxx ctermfg=24 guifg=#0075aa
    DevIconNfo { fg = "#ffffcd", },                                                        -- DevIconNfo     xxx ctermfg=230 guifg=#ffffcd
    DevIconObjectiveC { fg = "#599eff", },                                                 -- DevIconObjectiveC xxx ctermfg=111 guifg=#599eff
    DevIconTypeScriptDeclaration { fg = "#d59855", },                                      -- DevIconTypeScriptDeclaration xxx ctermfg=172 guifg=#d59855
    DevIconImg { fg = "#d0bec8", },                                                        -- DevIconImg     xxx ctermfg=181 guifg=#d0bec8
    DevIconSuo { fg = "#854cc7", },                                                        -- DevIconSuo     xxx ctermfg=98 guifg=#854cc7
    DevIconPart { fg = "#44cda8", },                                                       -- DevIconPart    xxx ctermfg=43 guifg=#44cda8
    DevIconTmux { fg = "#14ba19", },                                                       -- DevIconTmux    xxx ctermfg=34 guifg=#14ba19
    DevIconIgs { fg = "#839463", },                                                        -- DevIconIgs     xxx ctermfg=101 guifg=#839463
    DevIconElementary { fg = "#5890c2", },                                                 -- DevIconElementary xxx ctermfg=67 guifg=#5890c2
    DevIconIges { fg = "#839463", },                                                       -- DevIconIges    xxx ctermfg=101 guifg=#839463
    DevIconFreeCAD { fg = "#cb0d0d", },                                                    -- DevIconFreeCAD xxx ctermfg=160 guifg=#cb0d0d
    DevIconIge { fg = "#839463", },                                                        -- DevIconIge     xxx ctermfg=101 guifg=#839463
    DevIconClojureC { fg = "#8dc149", },                                                   -- DevIconClojureC xxx ctermfg=113 guifg=#8dc149
    DevIconTex { fg = "#3d6117", },                                                        -- DevIconTex     xxx ctermfg=22 guifg=#3d6117
    DevIconTor { fg = "#519aba", },                                                        -- DevIconTor     xxx ctermfg=74 guifg=#519aba
    DevIconIfb { fg = "#2b2e83", },                                                        -- DevIconIfb     xxx ctermfg=18 guifg=#2b2e83
    DevIconPop_OS { fg = "#48b9c7", },                                                     -- DevIconPop_OS  xxx ctermfg=73 guifg=#48b9c7
    DevIconXml { fg = "#e37933", },                                                        -- DevIconXml     xxx ctermfg=166 guifg=#e37933
    DevIconBazelBuild { fg = "#89e051", },                                                 -- DevIconBazelBuild xxx ctermfg=113 guifg=#89e051
    DevIconNswag { fg = "#85ea2d", },                                                      -- DevIconNswag   xxx ctermfg=112 guifg=#85ea2d
    DevIconIcs { fg = "#2b2e83", },                                                        -- DevIconIcs     xxx ctermfg=18 guifg=#2b2e83
    DevIconIcalendar { fg = "#2b2e83", },                                                  -- DevIconIcalendar xxx ctermfg=18 guifg=#2b2e83
    DevIconParabolaGNULinuxLibre { fg = "#797dac", },                                      -- DevIconParabolaGNULinuxLibre xxx ctermfg=103 guifg=#797dac
    DevIconVoid { fg = "#295340", },                                                       -- DevIconVoid    xxx ctermfg=23 guifg=#295340
    DevIconCpp { fg = "#519aba", },                                                        -- DevIconCpp     xxx ctermfg=74 guifg=#519aba
    DevIconCrdownload { fg = "#44cda8", },                                                 -- DevIconCrdownload xxx ctermfg=43 guifg=#44cda8
    DevIconEdn { fg = "#519aba", },                                                        -- DevIconEdn     xxx ctermfg=74 guifg=#519aba
    DevIconIxx { fg = "#519aba", },                                                        -- DevIconIxx     xxx ctermfg=74 guifg=#519aba
    DevIconCPlusPlus { fg = "#f34b7d", },                                                  -- DevIconCPlusPlus xxx ctermfg=204 guifg=#f34b7d
    DevIconDevuan { fg = "#404a52", },                                                     -- DevIconDevuan  xxx ctermfg=238 guifg=#404a52
    DevIconSystemVerilog { fg = "#019833", },                                              -- DevIconSystemVerilog xxx ctermfg=28 guifg=#019833
    DevIconNPMrc { fg = "#e8274b", },                                                      -- DevIconNPMrc   xxx ctermfg=197 guifg=#e8274b
    DevIconHuff { fg = "#4242c7", },                                                       -- DevIconHuff    xxx ctermfg=56 guifg=#4242c7
    DevIconConfigRu { fg = "#701516", },                                                   -- DevIconConfigRu xxx ctermfg=52 guifg=#701516
    DevIconSwift { fg = "#e37933", },                                                      -- DevIconSwift   xxx ctermfg=166 guifg=#e37933
    DevIconSvelte { fg = "#ff3e00", },                                                     -- DevIconSvelte  xxx ctermfg=196 guifg=#ff3e00
    DevIconScalaScript { fg = "#cc3e44", },                                                -- DevIconScalaScript xxx ctermfg=167 guifg=#cc3e44
    DevIconCue { fg = "#ed95ae", },                                                        -- DevIconCue     xxx ctermfg=211 guifg=#ed95ae
    DevIconMOV { fg = "#fd971f", },                                                        -- DevIconMOV     xxx ctermfg=208 guifg=#fd971f
    DevIconMageia { fg = "#2397d4", },                                                     -- DevIconMageia  xxx ctermfg=67 guifg=#2397d4
    DevIconPKGBUILD { fg = "#0f94d2", },                                                   -- DevIconPKGBUILD xxx ctermfg=67 guifg=#0f94d2
    DevIconTxt { fg = "#89e051", },                                                        -- DevIconTxt     xxx ctermfg=113 guifg=#89e051
    DevIconStyl { fg = "#8dc149", },                                                       -- DevIconStyl    xxx ctermfg=113 guifg=#8dc149
    DevIconMobi { fg = "#eab16d", },                                                       -- DevIconMobi    xxx ctermfg=215 guifg=#eab16d
    DevIconStp { fg = "#839463", },                                                        -- DevIconStp     xxx ctermfg=101 guifg=#839463
    DevIconKubuntu { fg = "#007ac2", },                                                    -- DevIconKubuntu xxx ctermfg=32 guifg=#007ac2
    DevIconAlpine { fg = "#0d597f", },                                                     -- DevIconAlpine  xxx ctermfg=24 guifg=#0d597f
    DevIconGentoo { fg = "#b1abce", },                                                     -- DevIconGentoo  xxx ctermfg=146 guifg=#b1abce
    DevIconPrusaSlicer { fg = "#ec6b23", },                                                -- DevIconPrusaSlicer xxx ctermfg=202 guifg=#ec6b23
    DevIconRedhat { fg = "#ee0000", },                                                     -- DevIconRedhat  xxx ctermfg=196 guifg=#ee0000
    DevIconapk { fg = "#34a853", },                                                        -- DevIconapk     xxx ctermfg=35 guifg=#34a853
    DevIconAss { fg = "#ffb713", },                                                        -- DevIconAss     xxx ctermfg=214 guifg=#ffb713
    DevIconSsa { fg = "#ffb713", },                                                        -- DevIconSsa     xxx ctermfg=214 guifg=#ffb713
    DevIconSrt { fg = "#ffb713", },                                                        -- DevIconSrt     xxx ctermfg=214 guifg=#ffb713
    DevIconMkv { fg = "#fd971f", },                                                        -- DevIconMkv     xxx ctermfg=208 guifg=#fd971f
    DevIconBz { fg = "#eca517", },                                                         -- DevIconBz      xxx ctermfg=214 guifg=#eca517
    DevIconUbuntu { fg = "#dd4814", },                                                     -- DevIconUbuntu  xxx ctermfg=196 guifg=#dd4814
    DevIconBoundaryRepresentation { fg = "#839463", },                                     -- DevIconBoundaryRepresentation xxx ctermfg=101 guifg=#839463
    DevIconZigObjectNotation { fg = "#f69a1b", },                                          -- DevIconZigObjectNotation xxx ctermfg=172 guifg=#f69a1b
    DevIconErl { fg = "#b83998", },                                                        -- DevIconErl     xxx ctermfg=163 guifg=#b83998
    DevIconHyperbolaGNULinuxLibre { fg = "#c0c0c0", },                                     -- DevIconHyperbolaGNULinuxLibre xxx ctermfg=250 guifg=#c0c0c0
    DevIconMd5 { fg = "#8c86af", },                                                        -- DevIconMd5     xxx ctermfg=103 guifg=#8c86af
    DevIconSolveSpace { fg = "#839463", },                                                 -- DevIconSolveSpace xxx ctermfg=101 guifg=#839463
    DevIconBlade { fg = "#f05340", },                                                      -- DevIconBlade   xxx ctermfg=203 guifg=#f05340
    DevIconBz2 { fg = "#eca517", },                                                        -- DevIconBz2     xxx ctermfg=214 guifg=#eca517
    DevIconYml { fg = "#6d8086", },                                                        -- DevIconYml     xxx ctermfg=66 guifg=#6d8086
    DevIconScss { fg = "#f55385", },                                                       -- DevIconScss    xxx ctermfg=204 guifg=#f55385
    DevIconBin { fg = "#9f0500", },                                                        -- DevIconBin     xxx ctermfg=124 guifg=#9f0500
    DevIconVala { fg = "#7239b3", },                                                       -- DevIconVala    xxx ctermfg=91 guifg=#7239b3
    DevIconEx { fg = "#a074c4", },                                                         -- DevIconEx      xxx ctermfg=140 guifg=#a074c4
    DevIconLiquid { fg = "#95bf47", },                                                     -- DevIconLiquid  xxx ctermfg=106 guifg=#95bf47
    DevIconArchlabs { fg = "#503f42", },                                                   -- DevIconArchlabs xxx ctermfg=238 guifg=#503f42
    DevIconGCode { fg = "#1471ad", },                                                      -- DevIconGCode   xxx ctermfg=32 guifg=#1471ad
    DevIconEnlightenment { fg = "#ffffff", },                                              -- DevIconEnlightenment xxx ctermfg=231 guifg=#ffffff
    DevIconQtile { fg = "#ffffff", },                                                      -- DevIconQtile   xxx ctermfg=231 guifg=#ffffff
    DevIconKali { fg = "#2777ff", },                                                       -- DevIconKali    xxx ctermfg=69 guifg=#2777ff
    DevIconSha224 { fg = "#8c86af", },                                                     -- DevIconSha224  xxx ctermfg=103 guifg=#8c86af
    DevIconGodotTextScene { fg = "#6d8086", },                                             -- DevIconGodotTextScene xxx ctermfg=66 guifg=#6d8086
    DevIconXcPlayground { fg = "#e37933", },                                               -- DevIconXcPlayground xxx ctermfg=166 guifg=#e37933
    DevIconKDEneon { fg = "#20a6a4", },                                                    -- DevIconKDEneon xxx ctermfg=37 guifg=#20a6a4
    DevIconAzureCli { fg = "#0078d4", },                                                   -- DevIconAzureCli xxx ctermfg=32 guifg=#0078d4
    DevIconpostmarketOS { fg = "#009900", },                                               -- DevIconpostmarketOS xxx ctermfg=28 guifg=#009900
    DevIconXInitrc { fg = "#e54d18", },                                                    -- DevIconXInitrc xxx ctermfg=196 guifg=#e54d18
    DevIconArtix { fg = "#41b4d7", },                                                      -- DevIconArtix   xxx ctermfg=38 guifg=#41b4d7
    DevIconFIGletFontFormat { fg = "#ececec", },                                           -- DevIconFIGletFontFormat xxx ctermfg=255 guifg=#ececec
    DevIconGitCommit { fg = "#f54d27", },                                                  -- DevIconGitCommit xxx ctermfg=196 guifg=#f54d27
    DevIconFIGletFontControl { fg = "#ececec", },                                          -- DevIconFIGletFontControl xxx ctermfg=255 guifg=#ececec
    DevIconFreeLosslessAudioCodec { fg = "#0075aa", },                                     -- DevIconFreeLosslessAudioCodec xxx ctermfg=24 guifg=#0075aa
    DevIconAsc { fg = "#576d7f", },                                                        -- DevIconAsc     xxx ctermfg=242 guifg=#576d7f
    DevIconFdmdownload { fg = "#44cda8", },                                                -- DevIconFdmdownload xxx ctermfg=43 guifg=#44cda8
    DevIconParrot { fg = "#54deff", },                                                     -- DevIconParrot  xxx ctermfg=45 guifg=#54deff
    DevIconJl { fg = "#a270ba", },                                                         -- DevIconJl      xxx ctermfg=133 guifg=#a270ba
    DevIconObjectFile { fg = "#9f0500", },                                                 -- DevIconObjectFile xxx ctermfg=124 guifg=#9f0500
    DevIconGo { fg = "#519aba", },                                                         -- DevIconGo      xxx ctermfg=74 guifg=#519aba
    DevIconSublime { fg = "#e37933", },                                                    -- DevIconSublime xxx ctermfg=166 guifg=#e37933
    DevIconConfiguration { fg = "#6d8086", },                                              -- DevIconConfiguration xxx ctermfg=66 guifg=#6d8086
    DevIconFavicon { fg = "#cbcb41", },                                                    -- DevIconFavicon xxx ctermfg=185 guifg=#cbcb41
    DevIconOpenBSD { fg = "#f2ca30", },                                                    -- DevIconOpenBSD xxx ctermfg=220 guifg=#f2ca30
    DevIconAudioInterchangeFileFormat { fg = "#00afff", },                                 -- DevIconAudioInterchangeFileFormat xxx ctermfg=39 guifg=#00afff
    DevIconFusion360 { fg = "#839463", },                                                  -- DevIconFusion360 xxx ctermfg=101 guifg=#839463
    DevIconNixOS { fg = "#7ab1db", },                                                      -- DevIconNixOS   xxx ctermfg=110 guifg=#7ab1db
    DevIconExe { fg = "#9f0500", },                                                        -- DevIconExe     xxx ctermfg=124 guifg=#9f0500
    DevIcon7z { fg = "#eca517", },                                                         -- DevIcon7z      xxx ctermfg=214 guifg=#eca517
    DevIcon3gp { fg = "#fd971f", },                                                        -- DevIcon3gp     xxx ctermfg=208 guifg=#fd971f
    DevIconEpub { fg = "#eab16d", },                                                       -- DevIconEpub    xxx ctermfg=215 guifg=#eab16d
    DevIconD { fg = "#427819", },                                                          -- DevIconD       xxx ctermfg=28 guifg=#427819
    DevIconDeepin { fg = "#2ca7f8", },                                                     -- DevIconDeepin  xxx ctermfg=39 guifg=#2ca7f8
    DevIconEmbeddedOpenTypeFont { fg = "#ececec", },                                       -- DevIconEmbeddedOpenTypeFont xxx ctermfg=255 guifg=#ececec
    DevIconManjaro { fg = "#33b959", },                                                    -- DevIconManjaro xxx ctermfg=35 guifg=#33b959
    DevIconDebian { fg = "#a80030", },                                                     -- DevIconDebian  xxx ctermfg=88 guifg=#a80030
    DevIconJsonc { fg = "#cbcb41", },                                                      -- DevIconJsonc   xxx ctermfg=185 guifg=#cbcb41
    DevIconCrystalLinux { fg = "#a900ff", },                                               -- DevIconCrystalLinux xxx ctermfg=129 guifg=#a900ff
    DevIconWeston { fg = "#ffbb01", },                                                     -- DevIconWeston  xxx ctermfg=214 guifg=#ffbb01
    DevIconWebpack { fg = "#519aba", },                                                    -- DevIconWebpack xxx ctermfg=74 guifg=#519aba
    DevIconCentos { fg = "#a2518d", },                                                     -- DevIconCentos  xxx ctermfg=132 guifg=#a2518d
    DevIconLXLE { fg = "#474747", },                                                       -- DevIconLXLE    xxx ctermfg=238 guifg=#474747
    DevIconVLC { fg = "#ee7a00", },                                                        -- DevIconVLC     xxx ctermfg=208 guifg=#ee7a00
    DevIconTFVars { fg = "#5f43e9", },                                                     -- DevIconTFVars  xxx ctermfg=93 guifg=#5f43e9
    DevIconSolidity { fg = "#519aba", },                                                   -- DevIconSolidity xxx ctermfg=74 guifg=#519aba
    DevIconLocOS { fg = "#fab402", },                                                      -- DevIconLocOS   xxx ctermfg=214 guifg=#fab402
    DevIconBigLinux { fg = "#189fc8", },                                                   -- DevIconBigLinux xxx ctermfg=38 guifg=#189fc8
    DevIconEbook { fg = "#eab16d", },                                                      -- DevIconEbook   xxx ctermfg=215 guifg=#eab16d
    DevIconTSConfig { fg = "#519aba", },                                                   -- DevIconTSConfig xxx ctermfg=74 guifg=#519aba
    DevIconAutoCADDxf { fg = "#839463", },                                                 -- DevIconAutoCADDxf xxx ctermfg=101 guifg=#839463
    DevIconProcfile { fg = "#a074c4", },                                                   -- DevIconProcfile xxx ctermfg=140 guifg=#a074c4
    DevIconAutoCADDwg { fg = "#839463", },                                                 -- DevIconAutoCADDwg xxx ctermfg=101 guifg=#839463
    DevIconTailwindConfig { fg = "#20c2e3", },                                             -- DevIconTailwindConfig xxx ctermfg=45 guifg=#20c2e3
    DevIconSway { fg = "#68751c", },                                                       -- DevIconSway    xxx ctermfg=64 guifg=#68751c
    DevIconArcoLinux { fg = "#6690eb", },                                                  -- DevIconArcoLinux xxx ctermfg=68 guifg=#6690eb
    DevIconKiCadSymbolTable { fg = "#ffffff", },                                           -- DevIconKiCadSymbolTable xxx ctermfg=231 guifg=#ffffff
    DevIconOpenTypeFont { fg = "#ececec", },                                               -- DevIconOpenTypeFont xxx ctermfg=255 guifg=#ececec
    DevIconDownload { fg = "#44cda8", },                                                   -- DevIconDownload xxx ctermfg=43 guifg=#44cda8
    DevIconIllumos { fg = "#ff430f", },                                                    -- DevIconIllumos xxx ctermfg=196 guifg=#ff430f
    DevIconSvelteConfig { fg = "#ff3e00", },                                               -- DevIconSvelteConfig xxx ctermfg=196 guifg=#ff3e00
    DevIconArchcraft { fg = "#86bba3", },                                                  -- DevIconArchcraft xxx ctermfg=108 guifg=#86bba3
    DevIconRakefile { fg = "#701516", },                                                   -- DevIconRakefile xxx ctermfg=52 guifg=#701516
    DevIconQuery { fg = "#90a850", },                                                      -- DevIconQuery   xxx ctermfg=107 guifg=#90a850
    sym "DevIconPy.typed" { fg = "#ffbc03", },                                             -- DevIconPy.typed xxx ctermfg=214 guifg=#ffbc03
    DevIconZsh { fg = "#89e051", },                                                        -- DevIconZsh     xxx ctermfg=113 guifg=#89e051
    DevIconGuix { fg = "#ffcc00", },                                                       -- DevIconGuix    xxx ctermfg=220 guifg=#ffcc00
    DevIconAOSC { fg = "#c00000", },                                                       -- DevIconAOSC    xxx ctermfg=124 guifg=#c00000
    DevIconMpv { fg = "#3b1342", },                                                        -- DevIconMpv     xxx ctermfg=53 guifg=#3b1342
    DevIconStaticLibraryArchive { fg = "#dcddd6", },                                       -- DevIconStaticLibraryArchive xxx ctermfg=253 guifg=#dcddd6
    DevIconBicepParameters { fg = "#9f74b3", },                                            -- DevIconBicepParameters xxx ctermfg=133 guifg=#9f74b3
    DevIconMaven { fg = "#7a0d21", },                                                      -- DevIconMaven   xxx ctermfg=52 guifg=#7a0d21
    DevIconClojureDart { fg = "#519aba", },                                                -- DevIconClojureDart xxx ctermfg=74 guifg=#519aba
    DevIconPuppyLinux { fg = "#a2aeb9", },                                                 -- DevIconPuppyLinux xxx ctermfg=145 guifg=#a2aeb9
    DevIconGarudaLinux { fg = "#2974e1", },                                                -- DevIconGarudaLinux xxx ctermfg=33 guifg=#2974e1
    DevIconPlatformio { fg = "#f6822b", },                                                 -- DevIconPlatformio xxx ctermfg=208 guifg=#f6822b
    DevIconAlmalinux { fg = "#ff4649", },                                                  -- DevIconAlmalinux xxx ctermfg=203 guifg=#ff4649
    DevIconCxxm { fg = "#519aba", },                                                       -- DevIconCxxm    xxx ctermfg=74 guifg=#519aba
    DevIconFreeBSD { fg = "#c90f02", },                                                    -- DevIconFreeBSD xxx ctermfg=160 guifg=#c90f02
    DevIconGz { fg = "#eca517", },                                                         -- DevIconGz      xxx ctermfg=214 guifg=#eca517
    DevIconHxx { fg = "#a074c4", },                                                        -- DevIconHxx     xxx ctermfg=140 guifg=#a074c4
    DevIconWindows { fg = "#00a4ef", },                                                    -- DevIconWindows xxx ctermfg=39 guifg=#00a4ef
    DevIconFedora { fg = "#072a5e", },                                                     -- DevIconFedora  xxx ctermfg=17 guifg=#072a5e
    DevIconUI { fg = "#0c306e", },                                                         -- DevIconUI      xxx ctermfg=17 guifg=#0c306e
    DevIconApple { fg = "#a2aaad", },                                                      -- DevIconApple   xxx ctermfg=248 guifg=#a2aaad
    DevIconHtm { fg = "#e34c26", },                                                        -- DevIconHtm     xxx ctermfg=196 guifg=#e34c26
    DevIconEndeavour { fg = "#7b3db9", },                                                  -- DevIconEndeavour xxx ctermfg=91 guifg=#7b3db9
    DevIconZst { fg = "#eca517", },                                                        -- DevIconZst     xxx ctermfg=214 guifg=#eca517
    DevIconYaml { fg = "#6d8086", },                                                       -- DevIconYaml    xxx ctermfg=66 guifg=#6d8086
    DevIconLXQtConfigFile { fg = "#0192d3", },                                             -- DevIconLXQtConfigFile xxx ctermfg=32 guifg=#0192d3
    DevIconopenSUSE { fg = "#6fb424", },                                                   -- DevIconopenSUSE xxx ctermfg=70 guifg=#6fb424
    DevIconReScript { fg = "#cc3e44", },                                                   -- DevIconReScript xxx ctermfg=167 guifg=#cc3e44
    DevIconLXDEConfigFile { fg = "#909090", },                                             -- DevIconLXDEConfigFile xxx ctermfg=246 guifg=#909090
    DevIconZig { fg = "#f69a1b", },                                                        -- DevIconZig     xxx ctermfg=172 guifg=#f69a1b
    DevIconMXLinux { fg = "#ffffff", },                                                    -- DevIconMXLinux xxx ctermfg=231 guifg=#ffffff
    DevIconXorgConf { fg = "#e54d18", },                                                   -- DevIconXorgConf xxx ctermfg=196 guifg=#e54d18
    DevIconKritarc { fg = "#f245fb", },                                                    -- DevIconKritarc xxx ctermfg=201 guifg=#f245fb
    DevIconAdvancedAudioCoding { fg = "#00afff", },                                        -- DevIconAdvancedAudioCoding xxx ctermfg=39 guifg=#00afff
    DevIconKritadisplayrc { fg = "#f245fb", },                                             -- DevIconKritadisplayrc xxx ctermfg=201 guifg=#f245fb
    DevIconMonkeysAudio { fg = "#00afff", },                                               -- DevIconMonkeysAudio xxx ctermfg=39 guifg=#00afff
    DevIconKdenliverc { fg = "#83b8f2", },                                                 -- DevIconKdenliverc xxx ctermfg=110 guifg=#83b8f2
    DevIconAndroid { fg = "#34a853", },                                                    -- DevIconAndroid xxx ctermfg=35 guifg=#34a853
    DevIconKdenliveLayoutsrc { fg = "#83b8f2", },                                          -- DevIconKdenliveLayoutsrc xxx ctermfg=110 guifg=#83b8f2
    DevIconApp { fg = "#9f0500", },                                                        -- DevIconApp     xxx ctermfg=124 guifg=#9f0500
    DevIconKDEglobals { fg = "#1c99f3", },                                                 -- DevIconKDEglobals xxx ctermfg=32 guifg=#1c99f3
    DevIconImage { fg = "#d0bec8", },                                                      -- DevIconImage   xxx ctermfg=181 guifg=#d0bec8
    DevIconKalgebrarc { fg = "#1c99f3", },                                                 -- DevIconKalgebrarc xxx ctermfg=32 guifg=#1c99f3
    DevIconTsx { fg = "#1354bf", },                                                        -- DevIconTsx     xxx ctermfg=26 guifg=#1354bf
    DevIconIfc { fg = "#839463", },                                                        -- DevIconIfc     xxx ctermfg=101 guifg=#839463
    DevIconCantorrc { fg = "#1c99f3", },                                                   -- DevIconCantorrc xxx ctermfg=32 guifg=#1c99f3
    DevIconXcLocalization { fg = "#2596be", },                                             -- DevIconXcLocalization xxx ctermfg=31 guifg=#2596be
    DevIconBazelWorkspace { fg = "#89e051", },                                             -- DevIconBazelWorkspace xxx ctermfg=113 guifg=#89e051
    DevIconGulpfile { fg = "#cc3e44", },                                                   -- DevIconGulpfile xxx ctermfg=167 guifg=#cc3e44
    DevIconVlang { fg = "#5d87bf", },                                                      -- DevIconVlang   xxx ctermfg=67 guifg=#5d87bf
    DevIconGIMP { fg = "#635b46", },                                                       -- DevIconGIMP    xxx ctermfg=240 guifg=#635b46
    DevIconGruntfile { fg = "#e37933", },                                                  -- DevIconGruntfile xxx ctermfg=166 guifg=#e37933
    DevIconMagnet { fg = "#a51b16", },                                                     -- DevIconMagnet  xxx ctermfg=124 guifg=#a51b16
    DevIconCjs { fg = "#cbcb41", },                                                        -- DevIconCjs     xxx ctermfg=185 guifg=#cbcb41
    DevIconMustache { fg = "#e37933", },                                                   -- DevIconMustache xxx ctermfg=166 guifg=#e37933
    DevIconPsd { fg = "#519aba", },                                                        -- DevIconPsd     xxx ctermfg=74 guifg=#519aba
    DevIconGitlabCI { fg = "#e24329", },                                                   -- DevIconGitlabCI xxx ctermfg=196 guifg=#e24329
    DevIconGradleWrapperProperties { fg = "#005f87", },                                    -- DevIconGradleWrapperProperties xxx ctermfg=24 guifg=#005f87
    Stl56889f_1e1517_bolditalic_ { gui = "bold,italic", bg = "#1e1517", fg = "#56889f", }, -- Stl56889f_1e1517_bolditalic_ xxx cterm=bold,italic gui=bold,italic guifg=#56889f guibg=#1e1517
    Stl68545b_1e1517_bolditalic_ { gui = "bold,italic", bg = "#1e1517", fg = "#68545b", }, -- Stl68545b_1e1517_bolditalic_ xxx cterm=bold,italic gui=bold,italic guifg=#68545b guibg=#1e1517
    Stldadada_1e1517_bolditalic_ { gui = "bold,italic", bg = "#1e1517", fg = "#dadada", }, -- Stldadada_1e1517_bolditalic_ xxx cterm=bold,italic gui=bold,italic guifg=#dadada guibg=#1e1517
    Stle77777_1e1517_bolditalic_ { gui = "bold,italic", bg = "#1e1517", fg = "#e77777", }, -- Stle77777_1e1517_bolditalic_ xxx cterm=bold,italic gui=bold,italic guifg=#e77777 guibg=#1e1517
    Stl1e1517_281e22__ { bg = "#281e22", fg = "#1e1517", },                                -- Stl1e1517_281e22__ xxx guifg=#1e1517 guibg=#281e22
    Stl_281e22__ { bg = "#281e22", },                                                      -- Stl_281e22__   xxx guibg=#281e22
    Stl281e22_9cbdc9__ { bg = "#9cbdc9", fg = "#281e22", },                                -- Stl281e22_9cbdc9__ xxx guifg=#281e22 guibg=#9cbdc9
    Stl9cbdc9_281e22__ { bg = "#281e22", fg = "#9cbdc9", },                                -- Stl9cbdc9_281e22__ xxx guifg=#9cbdc9 guibg=#281e22
    Stl56889f_281e22__ { bg = "#281e22", fg = "#56889f", },                                -- Stl56889f_281e22__ xxx guifg=#56889f guibg=#281e22
    Stl8b7079_281e22__ { bg = "#281e22", fg = "#8b7079", },                                -- Stl8b7079_281e22__ xxx guifg=#8b7079 guibg=#281e22
    Stl281e22_281e22__ { bg = "#281e22", fg = "#281e22", },                                -- Stl281e22_281e22__ xxx guifg=#281e22 guibg=#281e22
    Stl6d9f71_281e22__ { bg = "#281e22", fg = "#6d9f71", },                                -- Stl6d9f71_281e22__ xxx guifg=#6d9f71 guibg=#281e22
    Stlffd770_281e22__ { bg = "#281e22", fg = "#ffd770", },                                -- Stlffd770_281e22__ xxx guifg=#ffd770 guibg=#281e22
    sym "@ibl.indent.char.1" { gui = "nocombine", fg = "#393337", },                       -- @ibl.indent.char.1 xxx cterm=nocombine gui=nocombine guifg=#393337
    sym "@ibl.whitespace.char.1" { gui = "nocombine", fg = "#393337", },                   -- @ibl.whitespace.char.1 xxx cterm=nocombine gui=nocombine guifg=#393337
    sym "@ibl.scope.char.1" { gui = "nocombine", fg = "#8b7079", },                        -- @ibl.scope.char.1 xxx cterm=nocombine gui=nocombine guifg=#8b7079
    sym "@ibl.scope.underline.1" { gui = "underline", sp = "#8b7079", },                   -- @ibl.scope.underline.1 xxx cterm=underline gui=underline guisp=#8b7079
    UfoFoldedFg { fg = "#dadada", },                                                       -- UfoFoldedFg    xxx guifg=#dadada
    TodoFgTODO { fg = "#9cbdc9", },                                                        -- TodoFgTODO     xxx guifg=#9cbdc9
    TodoSignTODO { fg = "#9cbdc9", },                                                      -- TodoSignTODO   xxx guifg=#9cbdc9
    TodoBgFIX { gui = "bold", bg = "#e77777", fg = "#1e1517", },                           -- TodoBgFIX      xxx gui=bold guifg=#1e1517 guibg=#e77777
    TodoFgFIX { fg = "#e77777", },                                                         -- TodoFgFIX      xxx guifg=#e77777
    TodoSignFIX { fg = "#e77777", },                                                       -- TodoSignFIX    xxx guifg=#e77777
    TodoBgWARN { gui = "bold", bg = "#ffd770", fg = "#1e1517", },                          -- TodoBgWARN     xxx gui=bold guifg=#1e1517 guibg=#ffd770
    TodoFgWARN { fg = "#ffd770", },                                                        -- TodoFgWARN     xxx guifg=#ffd770
    TodoSignWARN { fg = "#ffd770", },                                                      -- TodoSignWARN   xxx guifg=#ffd770
    TodoFgNOTE { fg = "#88c3ab", },                                                        -- TodoFgNOTE     xxx guifg=#88c3ab
    TodoSignNOTE { fg = "#88c3ab", },                                                      -- TodoSignNOTE   xxx guifg=#88c3ab
    TodoBgPERF { gui = "bold", bg = "#dadada", fg = "#1e1517", },                          -- TodoBgPERF     xxx gui=bold guifg=#1e1517 guibg=#dadada
    TodoFgPERF { fg = "#dadada", },                                                        -- TodoFgPERF     xxx guifg=#dadada
    TodoSignPERF { fg = "#dadada", },                                                      -- TodoSignPERF   xxx guifg=#dadada
    TodoFgHACK { fg = "#ffd770", },                                                        -- TodoFgHACK     xxx guifg=#ffd770
    TodoSignHACK { fg = "#ffd770", },                                                      -- TodoSignHACK   xxx guifg=#ffd770
    TodoBgTEST { gui = "bold", bg = "#dadada", fg = "#1e1517", },                          -- TodoBgTEST     xxx gui=bold guifg=#1e1517 guibg=#dadada
    TodoFgTEST { fg = "#dadada", },                                                        -- TodoFgTEST     xxx guifg=#dadada
    TodoSignTEST { fg = "#dadada", },                                                      -- TodoSignTEST   xxx guifg=#dadada
    CmpItemAbbrDefault { fg = "#8b7079", },                                                -- CmpItemAbbrDefault xxx guifg=#8b7079
    CmpItemAbbrDeprecatedDefault { fg = "#63646a", },                                      -- CmpItemAbbrDeprecatedDefault xxx guifg=#63646a
    CmpItemAbbrMatchDefault { fg = "#8b7079", },                                           -- CmpItemAbbrMatchDefault xxx guifg=#8b7079
    CmpItemAbbrMatchFuzzyDefault { fg = "#8b7079", },                                      -- CmpItemAbbrMatchFuzzyDefault xxx guifg=#8b7079
    CmpItemKindDefault { fg = "#4fa9c6", },                                                -- CmpItemKindDefault xxx guifg=#4fa9c6
    CmpItemMenuDefault { fg = "#8b7079", },                                                -- CmpItemMenuDefault xxx guifg=#8b7079
    CmpItemMenu { CmpItemMenuDefault },                                                    -- CmpItemMenu    xxx links to CmpItemMenuDefault
    Stl8d8184_NONE__ { fg = "#8d8184", },                                                  -- Stl8d8184_NONE__ xxx guifg=#8d8184
    --Stlffd770_281e22__ { bg = "#281e22", fg = "#ffd770", },                                -- Stlffd770_281e22__ xxx guifg=#ffd770 guibg=#281e22
    NeoTreeMessage { gui = "italic", fg = "#3b2d31", },                                    -- NeoTreeMessage xxx gui=italic guifg=#3b2d31
    NeoTreeFadeText1 { fg = "#626262", },                                                  -- NeoTreeFadeText1 xxx guifg=#626262
    NeoTreeFadeText2 { fg = "#444444", },                                                  -- NeoTreeFadeText2 xxx guifg=#444444
    NeoTreeDotfile { fg = "#626262", },                                                    -- NeoTreeDotfile xxx guifg=#626262
    NeoTreeHiddenByName { NeoTreeDotfile },                                                -- NeoTreeHiddenByName xxx links to NeoTreeDotfile
    NeoTreeWindowsHidden { NeoTreeDotfile },                                               -- NeoTreeWindowsHidden xxx links to NeoTreeDotfile
    NeoTreeDimText { fg = "#3b2d31", },                                                    -- NeoTreeDimText xxx guifg=#3b2d31
    NeoTreeExpander { NeoTreeDimText },                                                    -- NeoTreeExpander xxx links to NeoTreeDimText
    NeoTreeModified { fg = "#d7d787", },                                                   -- NeoTreeModified xxx guifg=#d7d787
    NeoTreeFileStats { fg = "#47373b", },                                                  -- NeoTreeFileStats xxx guifg=#47373b
    NeoTreeFileStatsHeader { gui = "bold", fg = "#616369", },                              -- NeoTreeFileStatsHeader xxx gui=bold guifg=#616369
    Stl281e22_ffd770__ { bg = "#ffd770", fg = "#281e22", },                                -- Stl281e22_ffd770__ xxx guifg=#281e22 guibg=#ffd770
    NotifyTRACEBorder { fg = "#4f3552", },                                                 -- NotifyTRACEBorder xxx guifg=#4f3552
    Stl63646a_281e22__ { bg = "#281e22", fg = "#63646a", },                                -- Stl63646a_281e22__ xxx guifg=#63646a guibg=#281e22
    Stl68545b_281e22__ { bg = "#281e22", fg = "#68545b", },                                -- Stl68545b_281e22__ xxx guifg=#68545b guibg=#281e22
  }
end)
return theme
